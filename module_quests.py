...
# Note : This is defined as the first mayor quest in module_constants.py: 
 ...
 ("kidnapped_girl", "Porwanie", 0,
  "{!}Uwolnij Pana Iksińskiego z rąk bandytów i dowieź do Krakowa."
  ),
...
##     ),
 ...
 ("Kurland", "List",   0, "{!}Wymierz sprawiedliwość trzem oddziałom dezerterów."),

 

 ("Przesluchanie", "Przesłuchanie",   qf_random_quest, "{!}Do uzupełnienia."),
 ("Infiltracja", "Za garść złota",   0, "{!}Dowiedz się o położoniu kryjówki bandytów, a następnie rozbij bandę. Spróbuj zasięgnąć języka w karczmie {s20}."),
 ("gZdrada", "In tyrannos",   0, "{!}Pochwyć  targowiczan, którym udało się zbiec z Warszawy tuż przed rewolucją. Nim wyjadą poza granice kraju."),
 
  
 ("gList", "Szpiedzy targowicy",     0, "{!}Pozbądź się potencjalnych szpiegów. Powinni znajdować się w pobliżu kamienicy."),
 ("Wojna", "Wojna o niepodległość",     0, "{!}Pozbądź się potencjalnych szpiegów. Powinni znajdować się w pobliżu kamienicy."),
 ("Oboz_ros", "Obóz",     0, "{!}Odnajdź obóz powstańców rosyjskich, chodzą słuchy, że znajduje się nieopodal wązozu na północ od Lwowa."),
 ("Przygotowania", "Przygotowania",     0, "{!}Odnajdź obóz powstańców rosyjskich, chodzą słuchy, że znajduje się nieopodal wązozu na północ od Lwowa."),
 ("Oboz_weg", "Węgierscy konspiratorzy",     0, "{!}Odnajdź obóz powstańców węgierskich, chodzą słuchy, że znajduje się w górach na wschód od Koszyc."),
 ("Gaszek", "Gaszek", qf_random_quest, "{!}Do uzupełnienia."),
 ("Zdrada", "Zdrada", qf_random_quest, "{!}Do uzupełnienia."),
 ("RamieSprawiedliwosci", "Ramię Sprawiedliwości", qf_random_quest, "{!}Do uzupełnienia."),
 ("Obelga", "Obelga", qf_random_quest, "{!}Do uzupełnienia."),
 ("Dlugi", "Dług", qf_random_quest, "{!}Do uzupełnienia."),
 ("List", "List",     qf_random_quest, "{!}Do uzupełnienia."),
 ("Syn", "Bella matribus detestata",     qf_random_quest, "{!}Do uzupełnienia."),
 ("Donos", "Donos",   qf_random_quest, "{!}Do uzupełnienia."),

#Wieprz z trzody Epikura

 
 ("Patrole", "Wrogi patrol",   qf_random_quest, "{!}Zwiadowcy donieśli, że nieopodal pobliskiej wsi znajduje się wrogi patrol. Znajdź i zlikwiduj go."),
 ("DostarczBron2", "Dostarcz Broń",   qf_random_quest, "{!}Pochwyć i wykończ szpiega, który podejmie próbę przekazania wrogowi wiadomości, mogących doprowadzić rewolucję do klęski. Zaczaj się w pobliżu obozu i poczekaj aż się ujawni."),
 ("1000Zdrajca", "Zlikwiduj Dowódcę Garnizonu",   qf_random_quest, "{!}Zlikwiduj dowódcę garnizonu, aby zmniejszyć możliwości obronne miasta. Znajdziesz go w centrum miasta."),
 ("ZlotoMaruderow", "Złoto maruderów",   qf_random_quest, "{!}Znajdź obóz dezerterów i odbierz im zrabowane złoto."),
 ("DostarczBron", "Dostarcz Broń",   qf_random_quest, "{!}Pochwyć i wykończ szpiega, który podejmie próbę przekazania wrogowi wiadomości, mogących doprowadzić rewolucję do klęski. Zaczaj się w pobliżu obozu i poczekaj aż się ujawni."),
 ("Dezercja", "Dezercja",   0, "{!}Wymierz sprawiedliwość trzem oddziałom dezerterów."),
 ("Emisariusz", "Emisariusz",   qf_random_quest, "{!}Do uzupełnienia."),
 # ("3Zdrajcy", "Poplecznicy Targowicy",   0, "{!}Wymierz sprawiedliwość trzem oddziałom dezerterów."),
 ("Prusy", "Podburz miejscową ludność",   qf_random_quest, "{!}Udaj się na przedmieścia i podburzaj miejscową ludność, aby wywołać zamieszki i zmniejszyć potencjał obronny miasta."),

 ("Egzekucja", "Egzekucja",   qf_random_quest, "{!}Do uzupełnienia."),
 
 ("Kontruderzenie", "Kontruderzenie",   qf_random_quest, "{!}Zaatakuj i zniszcz oddział wroga dowodzony przez oficera wrogiej frakcji i weź jeńca."),
 ("Szpieg", "Szpieg",   qf_random_quest, "{!}Pochwyć i wykończ szpiega, który podejmie próbę przekazania wrogowi wiadomości, mogących doprowadzić rewolucję do klęski. Zaczaj się w pobliżu obozu i poczekaj aż się ujawni."),
 
 ("Manifesty", "Manifesty",   qf_random_quest, "{!}Zadaniem gracza jest dostarczenie manifestów Radiszczewa do dwóch pobliskich karczm. W tym celu zatrzymaj się w obu na minimum 7 godzin i wygłoś manifesty nakłaniające do walki."),
 
 # ("Patrole2", "Rozpoznanie ogniem",   qf_random_quest, "{!}Zlikwiduj trzy patrole w ciągu 7 dni."),
 # ("Spisek", "Magna culpa dolus est",   qf_random_quest, "{!}Wykryj spisek w obozie, dążący do przejęcia władzy nad rewolucją. "),
 # ("AtakNaTabory", "Atak na szlaki zaopatrzeniowe",   qf_random_quest, "{!}Zaatakuj dwa tabory przeciwnika, w celu osłabienia jego sił. "),
 ("RozpoznanieOgniem", "W potrzasku",   qf_random_quest, "{!}Zlikwiduj w ciągu 7 dni, trzy oddziały patrolujące tereny nieopodal miast lub zamków wroga."),

 
 ("Zasadzka", "Zasadzka",   qf_random_quest, "{!}Przeprowadź zasadzkę na konwój przewożący broń."),
 ("Przyczolek", "Kijów",   qf_random_quest, "{!}Wspomóż atak na Kijów przeprowadzany przez Atamana Hołowatego."),
 
 ("Przewrot", "Przewrót",   qf_random_quest, "{!}Do uzupełnienia."),
 ...
 ("PorwanaKobieta", "Porwanie",   qf_random_quest, "{!}Przeprowadź zasadzkę na konwój przewożący broń."),
 
 ("MayorEnd", "(null)",   0, "{!} "),

 ("Regiment", "Służba wzywa",   qf_random_quest, "{!} Odbierz powierzony ci regiment w obozie wojsk, nieopodal Krakowa."),
 ("NadszedlCzas", "Nadszedł czas",   qf_random_quest, "{!} Donieś do twierdzy Radom wieść o wybuchu powstania w Rzeczpospolitej."),
 ("Uciekinier", "Chorąży Świstacki",   qf_random_quest, "{!}Do uzupełnienia."),
  ("3Zdrajcy", "Poplecznicy Targowicy",   0, "{!}Wymierz sprawiedliwość trzem oddziałom dezerterów."),
  ("Uniwersal", "Uniwersał",   0, "{!}Wymierz sprawiedliwość trzem oddziałom dezerterów."),
 ("KosciuszkoEnd", "(null)",   0, "{!} "),
 ...
 ("szlachetni", "Wśród Szlachetnie Urodzonych", 0,
  "{!}Dowiedz swego urodzenia."
  ), 
  
 ("2Etap", "Węgierscy Konspiratorzy", 0,
  "{!}Odnajdź przywódców konspiracji w innych częściach kraju."
  ), 
 ("3Etap", "Insurekcja Kościuszkowska", 0,
  "{!}Odnajdź przywódców konspiracji w innych częściach kraju."
  ), 
  
 ("Challenge_1", "Pierwsza szabla", 0,"{!}"), 
 ("Challenge_2", "Trzy pojedynki na szable", 0,"{!}"), 
 ("Challenge_3", "Trzy pojedynki na pistolety", 0,"{!}"), 
 
 ("Challenge_end", "Pierwsza szabla", 0,"{!}"), 
 
 ("4Przejmij", "Przejmij {s1}", 0,"{!}"), 
 ("4Ofensywa", "Incerti sunt exitus belli.", 0,"{!}"), 
