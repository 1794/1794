...
game_menus = [
  ("start_game_0",menu_text_color(0xFF000000)|mnf_disable_all_keys,
    ...
    "none",
    [
		(assign, "$StartMenuState", 0),
		(assign, "$background_", -1),
		(assign, "$background_type", -1),
		(assign, "$background_idea", -1),
		(assign, "$background_prof", -1),
		(start_presentation, "prsnt_StartMenu"),
	],
    [
     ("continue",[],"Kontynuuj...",
...
##      ]),
  ...
  ("reports",0,
   "Renoma postaci: {reg5}^Honor: {reg6}^Morale: {reg8}^Limit wielkości oddziału: {reg7}^",
   "none",
   [(call_script, "script_game_get_party_companion_limit"),
    ...
    [
      ...
      ("view_morale_report",[],"Pokaż raport odnośnie morale.",
...
#NPC companion changes end
      ...
      ("view_faction_relations_report",[],"Pokaż raport relacji z frakcjami.",
       [(jump_to_menu, "mnu_faction_relations_report"),
        ]
       ),
      ("resume_travelling",[],"Powrót.",
       [(change_screen_return),
        ]
       ),
      ]
...
#             (start_presentation, "prsnt_custom_banner"),
  ...
  (
    "quest_krakow",mnf_disable_all_keys,
    "Kraków, dawna stolica Rzeczpospolitej.",
    "none",
    [],
    [
	  # ("pomin",[],"Ratuj Iksinskiego",[
	    # (assign, "$Prolog",10),
		# (set_spawn_radius, 4),
        # (spawn_around_party,"p_village_17","pt_bandits_awaiting_ransom"),
		...
		# (str_store_string, s2, "str_kidnapped_girl"),
		# (1,"script_start_quest", "qst_kidnapped_girl","trp_niemcewicz"),
		# (change_screen_map),
		
	  # ]),
	
      ("continue",[
	  # (display_message, "@AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", 0xd42121),
	  ],"Kontynuuj",
      [
	    # (display_message, "@BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB", 0xd42121),
	    (finish_mission),
		(change_screen_map),
		(assign, "$current_town","p_town_3"),
		(assign, "$g_encountered_party","p_town_3"),
		(assign, "$TownPart",1),
		(assign, "$OwnPlayerLocation", 1),
		(1,"script_cf_init_town_scene"),
		
		(try_begin),
			(eq, "$Prolog",6),
			(set_visitor, 12,"trp_player"),
			# (set_visitor, 48,"trp_tukaj"),
			# (set_visitor, 0,-1),
		(else_try),
			(set_visitor, 0,"trp_player"),
			# (set_visitor, 48,"trp_tukaj"),
  ...
  (
    "start_continue",0,
    "11 stycznia, Anno Domini 1794, karczma Otrzynówka nad rzeczką Krzką.",
    "none",
    [
	# (rest_for_hours, 8, 5000, 0),
	(eq, "$GameRestart",1),
	(assign, "$GameRestart", 0)
	],
    [
       ("continue",[],"Kontynuuj",
       [
         (1,"script_prepare_alley_to_fight"),
       ]),
#(ge, "$cheat_mode",1)
#      ("continue2",[],"Pomiń.",
#      [
...
#		   (assign,"$Prolog",2),
#		   (spawn_around_party, "p_karczma_f", "pt_ojciec"),
#		   (assign, "$OjciecParty",reg0),
#		   (party_set_flags, "$OjciecParty", pf_always_visible,1),
#		   (str_store_troop_name, s0, "trp_player"),
#		   (troop_set_name, "trp_find_item_cheat",s0),
#		   (party_add_members,"$OjciecParty","trp_find_item_cheat",1),
#		   (set_camera_follow_party,"$OjciecParty"),
#		   (call_script, "script_party_set_ai_state", "$OjciecParty", spai_holding_center, "p_town_3"),
#		   (change_screen_map),
#			]),
		]
  ),
  ...
  ("faction_relations_report",0,
   ...
   [(str_clear, s2),
    (try_for_range, ":cur_kingdom", kingdoms_begin, kingdoms_end),
      ...
      (try_begin),
        (ge, ":cur_relation", 90),
        (str_store_string, s3, "@Lojalni"),
      (else_try),
        (ge, ":cur_relation", 80),
        (str_store_string, s3, "@Oddani"),
      (else_try),
        (ge, ":cur_relation", 70),
        (str_store_string, s3, "@Lubiący"),
      (else_try),
        (ge, ":cur_relation", 60),
        (str_store_string, s3, "@Wdzięczni"),
      (else_try),
        (ge, ":cur_relation", 50),
        (str_store_string, s3, "@Przyjaźni"),
      (else_try),
        (ge, ":cur_relation", 40),
        (str_store_string, s3, "@Wspierający"),
      (else_try),
        (ge, ":cur_relation", 30),
        (str_store_string, s3, "@Życzliwi"),
      (else_try),
        (ge, ":cur_relation", 20),
        (str_store_string, s3, "@Współpracujący"),
      (else_try),
        (ge, ":cur_relation", 10),
        (str_store_string, s3, "@Akceptujący"),
      (else_try),
        (ge, ":cur_relation", 0),
        (str_store_string, s3, "@Obojętni"),
      (else_try),
        (ge, ":cur_relation", -10),
        (str_store_string, s3, "@Podejrzliwi"),
      (else_try),
        (ge, ":cur_relation", -20),
        (str_store_string, s3, "@Narzekający"),
      (else_try),
        (ge, ":cur_relation", -30),
        (str_store_string, s3, "@Wrogo nastawieni"),
      (else_try),
        (ge, ":cur_relation", -40),
        (str_store_string, s3, "@Niechętni"),
      (else_try),
        (ge, ":cur_relation", -50),
        (str_store_string, s3, "@Rozgniewani"),
      (else_try),
        (ge, ":cur_relation", -60),
        (str_store_string, s3, "@Nienawidzący"),
      (else_try),
        (ge, ":cur_relation", -70),
        (str_store_string, s3, "@Żądni zemsty"),
      (else_try),
        (str_store_string, s3, "@Mściwi"),
      (try_end),
      (str_store_faction_name, s4, ":cur_kingdom"),
      (assign, reg1, ":cur_relation"),
      (str_store_string, s2, "@{!}{s2}^{s4}: {reg1} ({s3})"),
    (try_end),
    (str_store_string, s1, "@Masz następujące relacje z frakcjami:^{s2}"),
  ...
  ("1794_cheats",mnf_scale_picture,
   ...
   [
			(assign, reg0, "$AIPower"),
			(assign, reg1, "$Etap4"),
			]+debugMessage("@AIPower: {reg0}, 4Etap: {reg1}")+[
   (jump_to_menu, "mnu_3etap_debug"),
   ]
	),
		("destroytarg",[
		(check_quest_active, "qst_Uciekinier"),
		],"teleport do swistackiego",
	   [
	   (party_get_position, pos0, "$uciekinierParty"),
	...
	("weather",[
		(assign, reg0, "$_POGODA"),
	],"{reg0?Wyłącz:Włącz} wiadomosci pogodowe",
   [
	(store_sub, "$_POGODA", 1, "$_POGODA"),
   ]
),
      ("waiterenity",[],"Podróż w czasie",
       [
           (assign,"$g_camp_mode", 1),
           (assign, "$g_infinite_camping", 0),
           (assign, "$g_player_icon_state", pis_camping),

           (rest_for_hours_interactive, 24 * 365, 100, 1), #rest while attackable
	...
	("powst",[],"Dodaj 10 fizylierów do oddziału",
   [
	(party_add_members, "p_main_party", "trp_lithuania_fusilier",10),
   ]
),

("choose_troop",[],"Lista NPC",
   [(jump_to_menu, "mnu_choose_troops_0"),]
),
("choose_scene",[],"Lista lokacji",
   [(jump_to_menu, "mnu_choose_scenes_0"),]),
   
	   # ("dffds",[],"regimenty",
	   # [
	   # (jump_to_menu, "mnu_oboz_wojsk"),
	   # ]),
	   
	   
	   ("aaaaaadddsaquest",[],"Pomiń zadanie",
	   [
		(try_for_range, ":q","qst_Zamek1","qst_quests_end"),
			(neq, ":q", "qst_3Etap"),
			(neq, ":q", "qst_2Etap"),
			(check_quest_active, ":q"),
			(1,"script_succeed_quest", ":q"),
			(1,"script_end_quest", ":q"),
		(try_end),
	   
	   ]),
	   
	   ("awans",[],"Awansuj na wyższy stopień oficerski",
	   [
	   (1,"script_awans"),
	   ]),
   ...
   [
    ...
    [
	   ...
	   [
		...
		("swafsf",[(lt, "$KosciuszkoQuest", 5)],"Koncowka 3 etapu",
	   [
		  (assign, "$KosciuszkoQuest", 5),
		  (try_for_range, ":center", centers_begin, centers_end),
			(store_faction_of_party, ":fac", ":center"),
			(eq, ":fac", Targowica),
			(call_script, "script_give_center_to_faction", ":center", Kosciuszkowcy),
		  (try_end),
		  # (call_script, "script_3Etap_quest_succeed", "qst_NadszedlCzas"),
		  (call_script, "script_3Etap_quest_succeed", "qst_3Zdrajcy"),
		  (call_script, "script_3Etap_quest_succeed", "qst_Infiltracja"),
		  (call_script, "script_3Etap_quest_succeed", "qst_Uniwersal"),
	   ...
	   [
		...
		("x1",[],"Rozpocznij (10)",
	   [
		(1,"script_addWinLevel", 10),
		(1,"script_powstanie"),
	   ]),
	   
		("x2",[],"Rozpocznij (20)",
	   [
		(1,"script_addWinLevel", 20),
		(1,"script_powstanie"),
	   ]),
	   
		("x3",[],"Rozpocznij (30)",
	   [
		(1,"script_addWinLevel", 30),
		(1,"script_powstanie"),
	   ]),
	   
		("x4",[],"Rozpocznij (40)",
	   [
		(1,"script_addWinLevel", 40),
		(1,"script_powstanie"),
	   ]),
	   
		("x5",[],"Rozpocznij (50)",
	   [
		(1,"script_addWinLevel", 50),
		(1,"script_powstanie"),
	   ]),
    ...
    ("etap2_wspolczynnik",mnf_scale_picture,
   "2 etap współczynnik",
   "none",
   [
    ],
  ...
  ("1794_mod_options",0,
   ...
   [],
	...
	# (try_begin),
		...
		# (str_store_string, s0, "@Wyłącz"),
	# (else_try),
		# (str_store_string, s0, "@Włącz"),
	# (try_end),
	# ],"{s0} widok obrażeń",
   # [
   # (store_sub, "$DamageScreenEnabled", 1, "$DamageScreenEnabled"),
 #  (1,"script_2Etap_quest_start", "qst_ZlotoMaruderow"),
   # ]
	# ),
	
	
	
	("duel_rank",[],"Rankingi pojedynków",
   [ 
		(jump_to_menu, "mnu_camp_rank"),
   ]),
   
("pogoda1",[],"Zwiększ czynnik pogodowy",
   [
	(val_add, "$Pogoda", 50),
	(val_clamp, "$Pogoda", 200, 1301),
	(assign, reg0, "$Pogoda"),
	(display_message, "@czynnik pogody: {reg0}"),
	(assign, ":rain_strength",0),
	(assign, ":rain_type",0),
	(assign, ":fog_distance",0),
    (try_begin),
		(is_between, "$Pogoda",350,650),###Deszcz
		(assign, ":rain_type",1),
		(store_sub, ":rain_strength", "$Pogoda", 350),
		(val_mul, ":rain_strength", 100),
		(val_div, ":rain_strength", 300),
		
		(assign, ":fog_distance", "$Pogoda"),
		(val_mul, ":fog_distance", 100),
		(val_div, ":fog_distance", 650),
		(store_random_in_range, ":r",30,70),
    (else_try),
		(is_between, "$Pogoda",0,350),###burza
		(assign, ":rain_type",1),
		(assign, ":rain_strength", 100),
		
		(assign, ":fog_distance", "$Pogoda"),
		(val_mul, ":fog_distance", 100),
		(val_div, ":fog_distance", 650),
		(val_max, ":fog_distance", 10),
		
		(store_random_in_range, ":r",70,101),
    (else_try),
		(is_between, "$Pogoda",650,1301),###Slonce
		(assign, ":rain_type",0),
		(assign, ":fog_distance",100000),
		(store_random_in_range, ":r",0,31),
    (try_end),
	(val_mul, ":r", 10),
	(assign, ":wind_power",":r"),
	(assign, ":wind_grass",":r"),
	(assign, ":waves",":r"),
	(assign, "$FogDistance", ":fog_distance"),
	(assign, "$RainStrength", ":rain_strength"),
	(assign, "$RainType", ":rain_type"),
    (assign,"$WindPower",":wind_power"),
    (assign,"$WindGrassPower",":wind_grass"),
    (assign,"$WavesAmount", ":waves"),
	(1,"script_set_weather"),
   ]
),

("pogoda2",[],"Zmniejsz czynnik pogodowy",
   [
	(val_add, "$Pogoda", -50),
	(val_clamp, "$Pogoda", 200, 1301),
	(assign, reg0, "$Pogoda"),
    ...
    (try_end),
	(store_mul, ":r2", ":r",10),
	(assign, ":wind_power",":r2"),
	(assign, ":wind_grass",":r2"),
	(assign, ":waves",":r"),
	(assign, "$FogDistance", ":fog_distance"),
	(assign, "$RainStrength", ":rain_strength"),
	(assign, "$RainType", ":rain_type"),
  ...
  ("1794_recruit_prisoners",mnf_scale_picture,
   "{reg20} jeńców jest gotowych na wcielenie do wojska. Czy napewno chcesz to zrobić?",
   "none",
   [
		(party_get_num_prisoner_stacks, ":num", "p_main_party"),
		(party_get_num_companions, ":current_size", "p_main_party"),
		(call_script, "script_GetPartyLimit", "p_main_party"),
		(assign, ":limit", reg0),
		# (val_sub, ":current_size", 1),
		(val_sub, ":limit", ":current_size"),
		
		(assign, ":non_hero", 0),
		(try_for_range, ":i", 0, ":num"),
			(party_prisoner_stack_get_troop_id, ":troop", "p_main_party", ":i"),
			(neg|troop_is_hero, ":troop"),
			(party_prisoner_stack_get_size, ":size", "p_main_party", ":i"),
			(val_add, ":non_hero", ":size"),
		(try_end),
		
		(assign, reg20, ":non_hero"),
    ...
    [
		   ...
		   [
				(party_get_num_prisoner_stacks, ":num", "p_main_party"),
				(troop_get_slot, ":pt", "trp_player", TroopsType),
				
				(party_template_get_slot, ":rtroop", ":pt", PTTroop),
				(call_script, "script_GetPartyLimit", "p_main_party"),
				(assign, ":limit", reg0),
				
				(try_for_range_backwards, ":i", 0, ":num"),
					(party_prisoner_stack_get_troop_id, ":troop", "p_main_party", ":i"),
					(neg|troop_is_hero, ":troop"),
					...
					(party_add_members, "p_main_party", ":rtroop", ":size"),
  ...
  ("camp",mnf_scale_picture,
   "Rozbiłeś obóz. Co chcesz zrobić?",
   "none",
   [
	...
	("1794_recruit",[
		(troop_slot_ge, "trp_player", TroopsType, 3),
		
		(party_get_num_companions, ":current_size", "p_main_party"),
		(call_script, "script_GetPartyLimit", "p_main_party"),
		(assign, ":limit", reg0),
		
		(lt, ":current_size", ":limit"),
		...
		(try_end),
		(gt, ":non_hero", 0),
	],"Wciel jeńców do oddziału",
   [(jump_to_menu, "mnu_1794_recruit_prisoners"),]
	),
   ...
   [(jump_to_menu, "mnu_1794_cheats"),]
	),

	("camp_1794fdsef",[
	],"Opcje modyfikacji",
   [(jump_to_menu, "mnu_1794_mod_options"),]
	),
	
	("debug_mode_switch",[
		(assign, reg5, "$_DEBUG"),
	],"{reg5?Wyłącz:Włącz} tryb debug",
	[
		(store_sub, "$_DEBUG", 1, "$_DEBUG"),
	]),
       ...
       ("camp_action",[(neg|check_quest_active, "qst_Manifesty")],"Manifesty",
        [
		(1,"script_2Etap_quest_start", "qst_Manifesty"),
         ]
        ),
      ("camp_wait_here",[],"Zaczekaj tu przez jakiś czas.",
       [
           (assign,"$g_camp_mode", 1),
           (assign, "$g_infinite_camping", 0),
           (assign, "$g_player_icon_state", pis_camping),
  ...
  (
   "camp_rank",mnf_disable_all_keys,
     "Który ranking chcesz wybrać?",
	  "none",
	  [(set_shader_param_int, "@disableUI", 1)],
	  [
	   ("1",[],"Broń palna",[
	     
	     (assign, "$duel_type", 2),
	     (assign, "$currentRankTroop", -1),
	     (assign, ":scene", "scn_duel_rank_pistol"),
	     (modify_visitors_at_site,":scene"),
         (reset_visitors),
         (set_visitor, 0, "trp_player"),
         (set_jump_mission,"mt_duel_rank"),
         (jump_to_scene,":scene"),
		 (change_screen_mission),
		 ]),
	   ...
	   ("2",[],"Broń biała",[
	     (assign, "$duel_type", 1),
	     (assign, "$currentRankTroop", -1),
	     (assign, ":scene", "scn_duel_rank_sword"),
	     (modify_visitors_at_site,":scene"),
         (reset_visitors),
         (set_visitor, 0, "trp_player"),
         (set_jump_mission,"mt_duel_rank"),
	   ...
	   # ("4",[],"test",[
	     # (troop_set_slot, "trp_temp_array_a", 1000, 2),
	     # (troop_set_slot, "trp_temp_array_a", 0, "qst_Infiltracja"),
	     # (troop_set_slot, "trp_temp_array_a", 1, "qst_Uciekinier"),
		 
	   
	     # (assign, "$SceneViewPresentation", "prsnt_4quests"),
	     # (assign, ":scene", "scn_quick_battle_1"),
	     # (modify_visitors_at_site,":scene"),
         # (reset_visitors),
         # (set_visitor, 0, "trp_player"),
         # (set_jump_mission,"mt_scene_view"),
         # (jump_to_scene,":scene"),
		 # (set_shader_param_int, "@disableUI", 1),
		 # (change_screen_mission),
		# ]),
	   ...
	   ("3",[],"Żaden.",[
	     (set_shader_param_int, "@disableUI", 0),
		 (change_screen_return),
		]),
	]),
  ...
  (
   "duel_menu_won_d",mnf_disable_all_keys,
     "Pojedynek wygrany!",
	  "none",
	  [],
	  [
	   ("continue",[],"Kontynuuj...",[
	     (party_get_slot, ":scene", slot_town_center, "$current_town"),
	     (party_get_slot, ":troop", SlotTownBrawler, "$current_town"),
	     (modify_visitors_at_site,":scene"),
         (reset_visitors),
         (set_visitor, 0, "trp_player"),
         (set_visitor, 1, ":troop"),
  ...
  (
   "duel_menu_lost_d",mnf_disable_all_keys,
     "Pojedynek przegrany!",
	  "none",
	  [],
	  [
  ...
  (
   "duel_menu_give_d",mnf_disable_all_keys,
     "You gave up.",
	  "none",
	  [],
	  [
  ...
  ("duel_end",mnf_disable_all_keys,
     "You {s0}!",
	  "none",
	  [
	  
		(troop_raise_proficiency_linear, "$DuelEnemyTroop",0,"$DuelW0"),
		(troop_raise_proficiency_linear, "$DuelEnemyTroop",1,"$DuelW1"),
		(troop_raise_proficiency_linear, "$DuelEnemyTroop",2,"$DuelW2"),
		(troop_raise_proficiency_linear, "$DuelEnemyTroop",3,"$DuelW3"),
		(troop_raise_proficiency_linear, "$DuelEnemyTroop",4,"$DuelW4"),
		(troop_raise_proficiency_linear, "$DuelEnemyTroop",5,"$DuelW5"),
		
	  (try_begin),
		(eq, "$duel_result",4),
		(str_store_string, s0,"@lost"),
	  (else_try),
		(eq, "$duel_result",1),
		(str_store_string, s0,"@won"),
	  (else_try),
		(eq, "$duel_result",2),
		(str_store_string, s0,"@gave up"),	  
	  (try_end),
	  
	  (try_begin),
		(eq, "$DuelEnemyTroop", "trp_uciekinier"),
		(jump_to_menu, "mnu_uciekinier_duel_end"),
	  (else_try),
	    (eq, "$DuelEnemyTroop", "trp_rywal"),
		(jump_to_menu, "mnu_zniewaga_end"),
	  (try_end),
	  (neq, "$DuelEnemyTroop", "trp_uciekinier"),
	  (neq, "$DuelEnemyTroop", "trp_rywal"),
	  
			(play_track, "track_bogus", 1),
		   ...
		   (val_mul, ":offset",2),
		   (val_add, ":offset",":t"),
		   (store_add, ":brawler", BrawlersBegin, ":offset"),
		   
		   (assign, "$talkto", ":brawler"),
		   (assign, "$talkto", BrawlersBegin),
		   (assign, "$OwnPlayerLocation", 1),
		   (1,"script_cf_init_town_scene"),
...
####POJEDYNEK END
  (
   "duel_menu_won",mnf_disable_all_keys,
     "Duel won.",
	  "none",
	  [],
	  [
 ...
 all your money, and ride off into the sunset....",
  ...
  (
    ...
    "{s2} Masz {reg10} jednostek gotowych do walki przeciwko ich {reg11}.",
    "none",
    [      
...
#          (try_end),
        ...
        (try_begin),
          ...
          (try_begin),
            ...
            (str_store_string, s2,"@Napotkałeś {s1}."),
          (else_try),
            (eq, "$g_encounter_type", enctype_fighting_against_village_raid),
            (str_store_party_name, s3, "$g_encounter_is_in_village"),
            (str_store_string, s2,"@Napotkałeś {s1}, gdy najeżdżał {s3}."),
          (else_try),
            (eq, "$g_encounter_type", enctype_catched_during_village_raid),
            (str_store_party_name, s3, "$g_encounter_is_in_village"),
            (str_store_string, s2,"@Zostałeś złapany przez {s1} podczas gdy najeżdżałeś {s3}."),
          (try_end),
        (try_end),
        (try_begin),
          (call_script, "script_party_count_members_with_full_health", "p_collective_enemy"),
    ...
    [
      ...
      "Rozkaż swoim jednostkom zaatakować bez ciebie.",
      [
...
#Troop commentary changes end
      ...
      ("encounter_retreat",[
         ...
         ],"Wycofaj się, zostawiając parę jednostek, aby zatrzymać pogoń.",[(jump_to_menu, "mnu_encounter_retreat_confirm"),]),
         
      ("encounter_surrender",[
         (eq,"$cant_leave_encounter", 1),
          ],"Poddaj się.",[(assign,"$g_player_surrenders",1)]),
    ]
  ),
  #a plan that will allow you and your men to escape with your lives
  (
    "encounter_retreat_confirm",0,
    "Jako dowódca oddziału z najwyższymi umiejętnościami taktycznymi,\
 ({reg2}), {reg3?wymyślasz:{s3} obmyślasz} plan, który pozwoli ocalić życie twoje i twoich ludzi,\
 musisz jednak zostawić {reg4} żołnierzy osłaniających wasz odwrót.",
    "none",
    [(call_script, "script_get_max_skill_of_player_party", "skl_tactics"),
     (assign, ":max_skill", reg0),
    ...
    [
      ("leave_behind",[],"Poświęcenie tych ludzi pozwoli zachować resztę.",[#Go on. The sacrifice of these men will save the rest.
      ...
      ("dont_leave_behind",[],"Nikogo nie pozostawimy.",[(jump_to_menu, "mnu_simple_encounter"),]),#No. We leave no one behind.
    ]
  ),
  (
    "encounter_retreat",0,
    "Rozkazujesz {reg4} żołnierzom zatrzymać wroga, podczas gdy wraz z resztą oddziały dokonasz odwrotu.",
    "none",
    [
     ],
    [
...
###Troop commentary changes end
  ...
  (
    ...
    "Twoje oddziały przygotowują się do zaatakowania wroga.",
    "none",
    [],
    [
      ("order_attack_begin",[],"Wydaj rozkaz do rozpoczęcia ataku.", 
      [
        (party_get_template_id, ":encountered_party_template", "$g_encountered_party"),		
        (try_begin),
		  (eq, ":encountered_party_template", "pt_village_farmers"),
      ...
      ("call_back",[],"Odwołaj ich.",[(jump_to_menu,"mnu_simple_encounter")]),
    ]
  ),
  (
    "order_attack_2",mnf_disable_all_keys,
    "{s4}^^Twoje straty: {s8}^^Straty wroga: {s9}",
    "none",
    [
    ...
    [
      ("order_attack_continue",[(eq, "$no_soldiers_left", 0)],"Rozkaż żołnierzom kontynuować atak.",[
          (jump_to_menu,"mnu_order_attack_2"),
          ]),
      ("order_retreat",[(eq, "$no_soldiers_left", 0)],"Odwołaj swoich żołnierzy.",[
          (jump_to_menu,"mnu_simple_encounter"),
          ]),
      ("continue",[(eq, "$no_soldiers_left", 1)],"Kontynuuj...",[
          (jump_to_menu,"mnu_simple_encounter"),
          ]),
    ]
  ),
  ...
  (
    ...
    "{s11}^^Twoje straty:{s8}{s10}^^Straty przeciwnika:{s9}",
    "none",
    [
     (try_begin),
       (eq, "$g_battle_result", 1),
     ...
     (try_begin),
       ...
       (str_store_string, s11, "@Zwyciężyłeś!"),
...
#       (call_script, "script_music_set_situation_with_culture", mtf_sit_victorious),
     ...
     (else_try),
       ...
       (str_store_string, s11, "@Bitwa przegrana. Twoje siły zostały zmiażdżone."),
       (set_background_mesh, "mesh_pic_defeat"),
     (else_try),
       (eq, "$g_battle_result", -1),
       (str_store_string, s11, "@Twoi towarzysze wynoszą cię z pola bitwy."),
       (troop_get_type, ":is_female", "trp_player"),
       (try_begin),
         (eq, ":is_female", 1),
     ...
     (else_try),
       (eq, "$g_battle_result", 1),
       (str_store_string, s11, "@Pokonałeś przeciwnika."),
       (try_begin),
         (gt, "$g_friend_fit_for_battle", 1),
         (set_background_mesh, "mesh_pic_victory"),
       (try_end),
     (else_try),
       ...
       (str_store_string, s11, "@Uciekłeś z pola bitwy."),
     (try_end),
#NPC companion changes begin
...
#NPC companion changes end
     ...
     (try_begin),
       ...
       (str_store_string, s10, "@^^Straty sojusznika:{s0}"),
     (try_end),
     ],
    [
      ("continue",[],"Kontynuuj...",[(jump_to_menu, "$g_next_menu"),]),
    ]
  ),
  ...
  (
    "total_victory", 0,
    "Zwycięstwo!",#You shouldn't be reading this... {s9}
    "none",
    [
		...
		(try_begin),
			...
			(try_for_range, ":i_stack", 0, ":num_stacks"),
			  ...
			  (neg|is_between, ":stack_troop", lords_begin, lords_end),
        ...
        (else_try),
          ...
          (try_for_range, ":stack_no", "$last_defeated_hero", ":num_stacks"), #May 31 bug note -- this now returns some heroes in victorious party as well as in the other party
			...
			(else_try),
              ...
              (str_store_string, s17, "@{s1} z {s3} zdołał uciec."),
			...
			(else_try),
              ...
              (call_script, "script_remove_troop_from_prison", ":stack_troop"),
              (troop_set_slot, ":stack_troop", slot_troop_leaded_party, -1),
			  # (check_quest_active, "qst_Kontruderzenie"),
        ...
        (else_try),
		  ...
		  (1,"script_party_destroyed_by", "$g_enemy_party","p_main_party"),
    ...
    [
      ("continue",[],"Kontynuuj...",[(change_screen_map)]),
        ]
  ),
  ...
  (
    ...
    [],
    [
      ("continue",[],"Kontynuuj...",[(jump_to_menu,"mnu_total_victory")]),
    ]
  ),
...
##            (eq, ":random_attribute", ca_intelligence),
  ...
  (
    "pre_join",0,
    "Stajesz się świadkiem bitwy w której walczy {s2} i {s1}.", # Decydujesz...
    "none",
    [
    ...
    [
      ...
      ("pre_join_help_defenders",[
          ...
          "Rush to the aid of the {s1}.",[
              ...
              (jump_to_menu,"mnu_join_battle")]),
      ("pre_join_leave",[],"Odejdź.",[(leave_encounter),(change_screen_return)]),
    ]
  ),
...
#      ("zendar_leave",[],"Leave town.",[[leave_encounter],[change_screen_return]]),
  ...
  (
    ...
    [
      ("enter",[],"Wejdź.",[(set_jump_mission,"mt_town_center"),(jump_to_scene,"scn_salt_mine"),(change_screen_mission)]),
      ("leave",[],"Opuść.",[(leave_encounter),(change_screen_return)]),
    ]
  ),
  (
...
#      ("enter",[],"Wejdź.",[[set_jump_mission,"mt_town_default"],[jump_to_scene,"scn_conversation_scene"],[change_screen_mission]]),
      ("enter",[],"Wejdź.",[(set_jump_mission,"mt_ai_training"),(jump_to_scene,"scn_four_ways_inn"),(change_screen_mission)]),
      ("leave",[],"Opuść.",[(leave_encounter),(change_screen_return)]),
    ]
  ),
  (
...
#    "Dhorak Keep, the stronghold of the bandits stands overlooking the barren wilderness.",
    ...
    [],
    [
      ("enter",[],"Wejdź.",[(set_jump_mission,"mt_town_center"),(jump_to_scene,"scn_dhorak_keep"),(change_screen_mission)]),
      ("leave",[],"Opuść.",[(leave_encounter),(change_screen_return)]),
    ]
  ),
...
##          (jump_to_menu, "mnu_total_victory"),
    ...
    [
      ("talk_to_siege_commander",[],"Zażądaj spotkania z przywódcą.",[
                                (call_script, "script_get_meeting_scene"), (assign, ":meeting_scene", reg0),
      ...
      ("join_siege_with_allies",[(neg|troop_is_wounded, "trp_player")], "Dołącz do ataku.",
       [
           (assign, "$g_joined_battle_to_help", 1), 
           (party_set_next_battle_simulation_time, "$g_encountered_party", -1),
           (try_begin),
           ...
           (try_begin),
			 ...
			 (assign, ":battle_scene", "scn_town_18_walls"),
           (else_try),
             # (party_get_slot, ":battle_scene", "$g_encountered_party", slot_castle_exterior),
			 # (party_get_slot, ":battle_scene", "$g_encountered_party", slot_castle_siege),
			 (assign, ":battle_scene", "scn_castle_45_siege"),
           (try_end),
           (call_script, "script_calculate_battle_advantage"),
           (val_mul, reg0, 2),
      ...
      ("join_siege_stay_back", [(call_script, "script_party_count_members_with_full_health", "p_main_party"),
       ...
       "Rozkaż żołnierzom przyłączyć się do ataku bez ciebie.",
       [
         (assign, "$g_joined_battle_to_help", 1), 
         (party_set_next_battle_simulation_time, "$g_encountered_party", -1),
         (try_begin),
    ...
    [
	      (try_begin),
           ...
           (else_try),
             (assign, reg6, 0),
           (try_end),
		   (str_store_string, s33, "@Jesteś w {reg6?mieście:zamku}"),
        (assign, "$g_enemy_party", "$g_encountered_party"),
        (assign, "$g_ally_party", -1),
...
#          (str_store_string, s3, "str_place_is_occupied_by_friendly"),
        ...
        (else_try),
          ...
          (reset_visitors),
          (set_visitor,0, "trp_player"),
          (set_visitor,17, "trp_iksinski"),
          (set_jump_mission, "mt_conversation_encounter"),
          (jump_to_scene, ":meeting_scene"),
          (assign, "$talk_context", tc_entering_center_quest_talk),
          (change_screen_map_conversation, "trp_iksinski"),
##        (else_try),
...
#             ]),
      ("nadszedlczas",[(check_quest_active, "qst_NadszedlCzas"),
                       (quest_slot_eq, "qst_NadszedlCzas", QuestSlot+0, "$g_encountered_party"),
						(eq,0,1),
        ],
       "Porozmawiaj z przywódcą.",[
                                                  (modify_visitors_at_site,"scn_castle_45_siege"),(reset_visitors),
                                                  (set_visitor,4,"trp_player"),
                                                  (set_visitor,5,"trp_przywodca_twierdzy"),
                                                  (set_jump_mission,"mt_conversation_encounter"),
                                                  (jump_to_scene,"scn_castle_45_siege"),
                                                  (change_screen_map_conversation, "trp_przywodca_twierdzy")
                ]),
      ...
      ("town_tavern",[
		...
		(display_message, "@2"),
		(eq, "$current_town", "$Etap4Region"),
		(display_message, "@3"),
		(party_slot_eq, "$current_town", Town4Etap, 0),
		(display_message, "@4"),
	  # (eq,0,1),
...
#          (scene_slot_eq, ":scene", slot_scene_visited, 1), #check if scene has been visited before to allow entry from menu. Otherwise scene will only be accessible from the town center.
       ...
       ,"Przeprowadź dywersję",
       [
		(troop_set_slot, "trp_temp_array_a", 1000, 2),
		(troop_set_slot, "trp_temp_array_a", 0, "qst_1000Zdrajca"),
		(troop_set_slot, "trp_temp_array_a", 1, "qst_Prusy"),
	    (assign, "$SceneViewPresentation", "prsnt_4quests"),
		(assign, ":scene", "scn_quick_battle_1"),
		(modify_visitors_at_site,":scene"),
		(reset_visitors),
...
#					(assign, "$g_last_assassination_attempt_perpetrator", ":lord"),
			 ...
			 (try_begin),
				...
				(store_sub, ":offset", "$current_town", "p_town_1"),
				(store_add, ":werbownik_troop", "trp_werbownik_a", ":offset"),
				
				(set_visitor, ":cur_entry", ":werbownik_troop"),
			...
			(try_begin),
				...
				(set_visitor, ":cur_entry", "trp_szpieg"),
				(val_add, ":cur_entry", 1),
            (try_end), 
			(try_begin),
				(eq, "$Prolog",7),
				(eq, "$current_town", "p_town_2"),
				(set_visitor, 29, "trp_odbiorca_listu"),
            (try_end), 
      ...
      ("approach_gates",[(eq,"$entry_to_town_forbidden",1),
                          (party_slot_eq,"$g_encountered_party", slot_party_type,spt_castle)],
       "Podejdź do bram i zawołaj straże.",[
...
##                                                   (change_screen_map_conversation, ":cur_guard")
      ...
      ("town_sneak",
       ...
       "Przebierz się i podejmij próbę wejścia.",
       [
         ...
         (val_mul, ":num_men", 2),
         (val_div, ":num_men", 3),
         (store_add, ":get_caught_chance", 0, ":num_men"),
         (store_random_in_range, ":random_chance", 0, 100),
         (try_begin),
           # (ge, ":random_chance", ":get_caught_chance"),
      ...
      ("castle_start_siege",
       [
		   (eq, "$Powstanie", 1),
	       (this_or_next|neq, "$g_encountered_party", "p_castle_19"),
		   (check_quest_active, "qst_NadszedlCzas"),
           ...
           (try_end),
		   (try_begin),
			 (eq, "$g_encountered_party_faction", Targowica),
			 (party_slot_eq, "$g_encountered_party", SlotSecondFaction, "fac_kingdom_5"),
			 (assign, reg7, 0),
		   (else_try),
		     (assign, reg7, 1),
		   (try_end),
           ],
       "{reg7?Oblegaj:Przejmij} {reg6?miasto:zamek}.",
       [
		 ...
		 (party_set_slot, "$g_encountered_party",SiegeState,-1),
		 (try_begin),
			(eq, reg7, 1),
		 ...
		 (else_try),
			...
			(1,"script_give_center_to_faction", "$g_encountered_party", "$players_kingdom"),
			(jump_to_menu, "mnu_break_menu"),
		(try_end),
         ]),
      ...
      ("castle_leave",[],"Opuść.",[(change_screen_return,0)]),
...
#                                                       (set_jump_mission,"mt_town_default"),
   ...
   (
    ...
    "Zbliżasz się do bramy. Straże na murach uważnie się obserwują.",
    "none",
    [
		(call_script, "script_set_town_picture"),
    ],
 ...
 and the guard sergeant informs you that his men will fire if you attempt to come any closer.",
  ...
  (
    ...
    [
      ("town_werbownik",[
          (party_slot_eq,"$current_town",slot_party_type, spt_town),
		  (store_sub, ":town", "$current_town","p_town_1"),
          (store_add, ":trp", "trp_werbownik_a", ":town"),
		  (troop_slot_eq, ":trp",999,2),
#          (party_get_slot, ":scene", "$current_town", slot_town_tavern),
#          (scene_slot_eq, ":scene", slot_scene_visited, 1), #check if scene has been visited before to allow entry from menu. Otherwise scene will only be accessible from the town center.
          ]
       ,"z werbownikiem",
...
#					(assign, "$g_last_assassination_attempt_perpetrator", ":lord"),
            ...
            (try_end), 
			 ...
			 (store_sub, ":town", "$current_town","p_town_1"),
			 (store_add, ":trp", "trp_werbownik_a", ":town"),
			 (start_mission_conversation,":trp"),
           (try_end),
        ],"Door to the tavern."),   
  ...
  (
    "3Zdrajcy",0,
    "Straże będące w pobliżu dopomogły w pochwyceniu jednego z konfederatów...",
    "none",
    [
			(quest_get_slot, ":val", "qst_3Zdrajcy", QuestSlot+0),
			(val_add, ":val", 1),
			(assign, reg0, ":val"),
			(display_message, "@Złapałeś {reg0}/3 zdrajców", 0x007f00),
			(quest_set_slot, "qst_3Zdrajcy", QuestSlot+0, ":val"),
			(try_for_range, ":i", QuestSlot+1, QuestSlot+4),
				(quest_slot_eq, "qst_3Zdrajcy", ":i", "$current_town"),
				(quest_slot_eq, "qst_3Zdrajcy", ":i", "$current_town"),
				(quest_set_slot, "qst_3Zdrajcy", ":i", -1),
			(try_end),
			(eq, ":val", 3),
			(try_begin),
				(eq, "$Powstanie", 1),
				(1,"script_3Etap_quest_succeed", "qst_3Zdrajcy"),
			(else_try),
				(1,"script_succeed_quest", "qst_3Zdrajcy"),
				(1,"script_end_quest", "qst_3Zdrajcy"),
			(try_end),
	],
    [
      ("continue",[],
   ...
   (
    ...
    [
        ...
        (try_begin),
		  ...
		  # (display_message, "@DEBUGL- party_set_faction 1"),
		  ...
		  # (display_message, "@DEBUGL- party_set_faction 2"),
		...
		(try_begin),
			(party_slot_eq, "$g_encountered_party",SiegeState,-1),
			(party_set_slot, "$g_encountered_party",SiegeState,0),
		(try_end),
		
		(party_get_slot,reg6,"$g_encountered_party",SiegeState),
		(try_begin),(ge, "$cheat_mode", 1),
		(display_message, "@{reg6}",0xff0000),
		(try_end),
    ],
    [
      ("siege_request_meeting",[(eq, "$cant_talk_to_enemy", 0)],"Negocjuj z dowódcą twierdzy.", [
      ...
      ("siege_dywersja",[(eq, "$Szpieg", 1),
	  (neg|party_slot_eq,"$g_encountered_party",BramaWysadzona,1),
	  ],"Przeprowadź dywersję (30% szans na powodzenie).", [
	      (party_remove_members, "p_main_party","trp_szpieg",1),
          (assign, "$Szpieg",Dywersja),
		  (assign, "$SzpiegCel","$g_encountered_party"),
       ]),
		   
      ("call_art",[(party_slot_eq, "$g_encountered_party",SiegeState,0)],"Pchnij gońca po artylerię", [
	  (assign, ":distance",999999),

	  # (store_faction_of_party, ":faction","p_main_party"),
	  (assign, ":town_no",-1),
		  (try_for_range, ":town", towns_begin, towns_end),
		    (neq, ":town", "p_main_party"),
			(store_faction_of_party, ":fac",":town"),
			(eq, ":fac", "$players_kingdom"),
			(party_get_position, pos0, ":town"),
		 ...
		 (try_end),
		  ...
		  (str_store_party_name, s1, "$g_encountered_party"),
		  (display_message, "@Spawnuje artylerzystow obok {s0}|{s1}",0xff0000),
		  (store_sub, ":offset", "$players_kingdom","fac_kingdom_1"),
		  (store_add, ":troop", "trp_polish_cannoneer", ":offset"),
		  (party_add_members, reg0,":troop",9),
		  
		  (try_begin),
			(neq, "$g_encountered_party", "p_main_party"),
			(party_set_slot, ":party", 20,"$g_encountered_party"),
			(party_set_slot, ":party", 20,"$g_encountered_party"),
			(party_set_slot, ":party", 20,"$g_encountered_party"),
			(party_get_slot, reg1, ":party", 20),
			(str_store_party_name, s6, reg1),
			# (display_message,"@{s6}ehhhh"),
		    (call_script, "script_party_set_ai_state", ":party", spai_retreating_to_center, "$g_encountered_party"),
			(party_set_ai_object,":party","$g_encountered_party"),
			(party_set_slot, "$g_encountered_party", SiegeState,1),
		  (else_try),
		    (ge, "$cheat_mode", 1),
		    (display_message, "@cos sie spierniczylo.... error kurde...",0xff0000),
		  (try_end),
          (jump_to_menu,"mnu_castle_besiege"),
          ]),
      ...
      ("wait_24_hours",[],"Czekaj.", [
          ...
          (rest_for_hours_interactive, 100, 5, 1), #rest while attackable
      ...
      ("castle_lead_attack",
       [
         (neg|troop_is_wounded, "trp_player"),
		 (party_slot_eq, "$g_encountered_party",SiegeState,2),
         # (ge, "$g_siege_method", 1),
         # (gt, "$g_friend_fit_for_battle", 3),
         # (store_current_hours, ":cur_hours"),
         # (ge, ":cur_hours", "$g_siege_method_finish_hours"),
       ],
       "Poprowadź żołnierzy do szturmu.", 
       [
           (try_begin),

		     (party_slot_eq, "$g_encountered_party", BramaWysadzona,1),
			 (party_get_slot, ":battle_scene", "$g_encountered_party", SiegeSabotageScene),
           (else_try),
             (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
             (party_get_slot, ":battle_scene", "$g_encountered_party", slot_town_walls),
			 (assign, ":battle_scene", "scn_town_18_walls"),
      ...
      ("attack_stay_back",
       ...
       "Wydaj żołnierzom rozkaz szturmu...", [(assign, "$cant_talk_to_enemy", 0),(jump_to_menu,"mnu_castle_attack_walls_simulate")]),
      ...
      ("cheat_castle_lead_attack",[(eq, "$cheat_mode", 1),
       ...
       [
		 ...
		 (party_set_slot, "$g_encountered_party",SiegeState,2),
         (jump_to_menu, "mnu_castle_besiege"),
       ]),
...
# TODO: To use for the future:
 ...
 Now the attackers are pouring into the streets. IF you can defeat them, you can perhaps turn the tide and save the day."),
        ...
        (else_try),
          ...
          (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
          (try_begin),
            (this_or_next|eq, "$g_siege_battle_state", 0),
...
##      ("call_soldiers_back",[(eq, "$no_soldiers_left", 0)],"Call your soldiers back.",[(jump_to_menu,"mnu_castle_outside")]),
      ("continue",[],"Kontynuuj...",[(jump_to_menu,"mnu_castle_besiege")]),
    ]
  ),
  
   (
    "castle_attack_walls_with_allies_simulate",mnf_scale_picture|mnf_disable_all_keys,
    "{s4}^^Twoje straty: {s8}^^Straty sojusznika: {s9}^^Straty wroga: {s10}",
    "none",
    [
        (try_begin),
          (set_background_mesh, "mesh_pic_siege_attack"),
...
##        (display_message, "@{!}enemy_party={reg3} ally_party={reg4}"),
    ...
    [
      ("continue",[],"Kontynuuj...",[(jump_to_menu,"mnu_besiegers_camp_with_allies")]),
    ]
  ),
  ...
  (
    ...
    [
		...
		(try_end),
		(display_message, "@script_give_center_to_faction 0"),
...
{reg1? You may station troops here to defend it against enemies who may try to recapture it. Also, you should select now whether you will hold the {reg2?town:castle} yourself or give it to a faithful vassal...:}",# Only visible when castle is taken without being a vassal of a kingdom.
  ...
  (
    ...
    "{s3} został zdobyty! Przejmujesz kontrolę nad fortecą i pobliskimi terenami. Zaczekaj na obsadzenie zamku nowymi regimentami lub zostaw własne jednostki, aby zapobiec odbiciu fortyfikacji.\
 Wieść o zwycięstwie została wysłane do {s9}. {s5}",
    "none",
    [
        (str_store_party_name, s3, "$g_encountered_party"),
        (str_clear, s5),
    ...
    [
      ...
      ("castle_taken_no_claim",[],"Kontynuuj.",
       [
...
##  ),
  ...
  (
    ...
    "{s1} przeprowadza atak na {s2}. Stosunek liczebności twojego oddziału do oddziału przeciwnika wynosi {reg10} do {reg11}. Decydujesz się...",
    "none",
    [
        (select_enemy,1),
        (assign, "$g_enemy_party", "$g_encountered_party_2"),
  ...
  ##            (lt, ":min_enemy_str", "$g_friend_fit_for_battle"),
            ...
            (try_begin),
              ...
              (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
                ...
                (party_slot_eq, ":party_no", slot_party_ai_substate, 1),
                (call_script, "script_party_set_ai_state", ":party_no", spai_undefined, -1),
				# (display_message, "@DEBUGL: script_party_set_ai_state 1", 0xd42121),
                (call_script, "script_party_set_ai_state", ":party_no", spai_besieging_center, "$g_encountered_party"),
              (try_end),
              (display_message, "@Wróg został zmuszony do ucieczki. Szturm dobiega końca, lecz oblężenie nadal trwa."),
    ...
    [
      ...
      ("siege_defender_troops_join_battle",[(call_script, "script_party_count_members_with_full_health", "p_main_party"),
                                            ...
                                            (ge, reg0, 3)],
          "Rozkazać żołnierzom, by dołączyli do walki bez ciebie.",[
              (party_set_next_battle_simulation_time, "$g_encountered_party", -1),
              (select_enemy,1),
              (assign,"$g_enemy_party","$g_encountered_party_2"),
              (assign,"$g_ally_party","$g_encountered_party"),
  ...
  (
    ...
    [
       ...
       ("uniwersal",[(check_quest_active, "qst_Uniwersal"),
        (this_or_next|quest_slot_eq, "qst_Uniwersal", QuestSlot+0, "$current_town"),
        (this_or_next|quest_slot_eq, "qst_Uniwersal", QuestSlot+1, "$current_town"),
        (quest_slot_eq, "qst_Uniwersal", QuestSlot+2, "$current_town"),

        ],"Wygłoś uniewersał.",[
          (change_screen_map),
          (rest_for_hours, 24, 10, 1),
          (assign, "$Uniwersal","$g_encountered_party"),
        ]),




      ("village_leave",[],"Opuść...",[(change_screen_return,0)]),
...
#           (party_add_template, "$current_town", "pt_villagers_in_raid"),
      ...
      ("forget_it",[],
      "Odejdź.",[(jump_to_menu,"mnu_village")]),
    ],
  ),
 ...
 but luckily none of the wounds appear to be lethal.",
    ...
    [],
    [
      ("continue",[],"Kontynuuj...",[(jump_to_menu, "mnu_village"),]),
    ],
  ),
 ...
 but they offer you all they can find.",
    ...
    [(party_get_slot, ":bandit_troop", "$g_encountered_party", slot_village_infested_by_bandits),
     (party_set_slot, "$g_encountered_party", slot_village_infested_by_bandits, 0),
     (party_clear, "p_temp_party"),
     (party_add_members, "p_temp_party", ":bandit_troop", 0),
 ...
 {reg5} denars and will take {reg6} days.",
    ...
    [
      ...
      ("forget_it",[(store_troop_gold, ":cur_gold", "trp_player"),
                    (ge, ":cur_gold", reg5)],
       "Odejdź.", [(jump_to_menu,"mnu_center_manage")]),
...
#      (call_script, "script_loot_player_items", 0),
 ...
 then vanish into the night. They have left you alive, if only barely."),
    ...
    [
      ("continue",[],"Kontynuuj...",[(change_screen_return)]),
    ],
  ),
 ...
 Or perhaps, it was given to them by someone who wanted to arrange a suitable ending to your life.",
   ...
   (
    ...
    [
      ...
      ("forget_it",[],"Odejdź.",[(change_screen_return)]),
    ],
  ),
  ...
  (
   ...
   (
    ...
    [
      ...
      ("forget_it",[],"Odejdź.",[(jump_to_menu, "mnu_village_hostile_action")]),
    ],
  ),
...
#NPC companion changes end
  ...
  (
    ...
    "Witaj w mieście {s2}, które należy do frakcji {s9}.",
    "none",
    [     
	
	# (assign, reg0, "$current_town"),
	# (assign, reg1, "$g_encountered_party"),
	(party_get_slot, reg0, "$g_encountered_party", SiegeState),
	]+debugMessage("@{reg0}")+[
	...
	(try_begin),
		...
		(quest_slot_eq, "qst_Emisariusz", slot_quest_current_state, 10),
		(1,"script_succeed_quest", "qst_Emisariusz"),
		(1,"script_end_quest", "qst_Emisariusz"),
		(troop_add_gold, "trp_player", 3000),
		# (1,"script_addWinLevel", 2),
		(party_remove_prisoners, "p_main_party", "trp_qemisariuszt1",1),
	(else_try),
		(eq, "$current_town", "p_town_1"),
		(check_quest_active, "qst_Emisariusz"),
		(quest_slot_eq, "qst_Emisariusz", slot_quest_current_state, 11),
		(1,"script_succeed_quest", "qst_Emisariusz"),
		(1,"script_end_quest", "qst_Emisariusz"),
		# (1,"script_addWinLevel", 2),
	(try_end),
	...
	# (try_begin),
		...
		# (check_quest_active, "qst_Oboz_weg"),
		# (1,"script_succeed_quest","qst_Oboz_weg"),
		# (1,"script_end_quest","qst_Oboz_weg"),
	# (try_end),
	
	(assign, "$talkto",-1),
	(assign, "$EnablePassage",0),
	(assign, "$TownPart",0),
        (try_begin),
          (eq, "$sneaked_into_town", 1),
...
#		(try_end),
    ...
    [
      ...
      ("join_tournament", [(is_currently_night),
	  (check_quest_active, "qst_RozpoznanieOgniem"),
	  (quest_slot_eq, "qst_RozpoznanieOgniem", slot_quest_current_state, 0),
	  (quest_slot_eq, "qst_RozpoznanieOgniem", slot_quest_target_party, "$current_town"),
	  ]
       ,"Podejmij próbę odnalezienia emisariusza.",
       [
           (assign, ":scene", "scn_qemisariusz"),
		   (quest_set_slot, "qst_RozpoznanieOgniem", slot_quest_current_state, 1),
		   (modify_visitors_at_site, ":scene"),
		   (reset_visitors,0),
		   (set_visitor, 1, "trp_player"),
		   (set_jump_mission, "mt_qrozpoznanieogniem"),
		   (jump_to_scene, ":scene"),
		   (set_rain, 1, 100),
		   (change_screen_mission),
        ]),
      ...
      ("town_castle",[        
	     (eq, "$current_town", "p_town_16"),
		 (troop_slot_ge, "trp_dowodca2", TroopDialogLevel, 1),
		 (eq, "$IgnacAvailable", 1),
          ],"Idź do Ignaca Martinovics",
       [        
	   (try_begin),
	      (eq, "$current_town", "p_town_16"),
		  (eq, "$IgnacAvailable", 1),
		  (troop_slot_ge, "trp_dowodca2", TroopDialogLevel, 1),   	   
          (modify_visitors_at_site, "scn_budapeszt_kamienica_interior"),
          (reset_visitors),
		  (set_visitor, 0, "trp_player"),
		  (set_visitor, 1, "trp_dowodca2"),
		  (set_passage_menu, "mnu_martinovics"),
	      (set_jump_mission,"mt_kochanka"),
	      (jump_to_scene,"scn_budapeszt_kamienica_interior"),
		  (change_screen_mission),
		(else_try),
             (display_message,"str_door_locked",0xFFFFAAAA),
		(try_end),
      ...
      [                        
        ...
        (eq, "$sneaked_into_town",1),
           # (party_get_slot, ":town_scene", "$current_town", slot_town_center),
		   # (assign, reg0, ":town_scene"),
		   # (display_message, "@DEBUGYYY: {reg0}",0x237100),
      ],
      "Idź do centrum miasta...",
       [
	   (assign, "$TownPart",1),
	   (assign, "$OwnPlayerLocation", 0),
	   (1,"script_cf_init_town_scene"),
	   (1,"script_start_town_scene"),
		# (try_begin),
			# (is_currently_night),
			# (jump_to_menu, "mnu_town_night"),
		# (try_end),
		# (neg|is_currently_night),
         ...
         # (try_begin),
		   ...
		   # (try_begin),
			  ...
			  # (set_visitor, 5, "trp_carski_urzednik"),
		   # (try_end),
           # (set_jump_mission,"mt_town_center"),
...
#					(assign, "$g_last_assassination_attempt_perpetrator", ":lord"),
      ...
      ("town_arena",
	   ...
	   # (eq, 0,1),
	   (party_slot_eq,"$current_town",slot_party_type, spt_town),
	   (1,"script_getLoverInTown", "$current_town"),
	   (neq, reg0, -1), 
	   (assign, reg10, reg0),
	   ],
       "Odwiedź kochankę",
       [
	    (troop_get_slot, ":dna", "trp_dna",reg10),
	    (troop_get_slot, ":trp", "trp_group_troop",reg10),
		# (store_sub, ":offset","$current_town",towns_begin),
		(store_add, ":scene", "scn_town_1_flat", 0),
        (modify_visitors_at_site, ":scene"),
        (reset_visitors),
		(str_store_troop_name, s0, ":trp"),
		(assign, reg0, ":trp"),
		(assign, reg1, ":dna"),
		(try_begin),(ge, "$cheat_mode", 1),(display_message, "@{reg0} {s0} dna: {reg1}",0x0000ff),(try_end),
		(set_visitor, 0, "trp_player"),
		(set_visitor, 1, ":trp",":dna"),
	      (set_jump_mission,"mt_kochanka"),
	      (jump_to_scene,":scene"),
		  (change_screen_mission),
		  (assign, "$Kochanka", 2),
      ...
      [
         ...
         (this_or_next|eq, "$EnablePassage",1),
         (eq, "$Prolog",13),
		 (neq, "$Rosja",1),
		 (neq, "$Wegry",1),
		 (lt, "$Prolog", 14),
	   ...
	   (try_begin),
		    (lt, "$Prolog", 14),
             (modify_visitors_at_site, "scn_krakow_kamienica_interior"),
             (reset_visitors),
			 (try_begin),
			     (eq, "$Prolog",13),
				 (set_visitor, 0, "trp_player"),
				 (set_visitor, 1, "trp_jan"),
				 (set_visitor, 2, "trp_ojciec"),
				 # (set_visitor, 3, "trp_dama"),
				 (set_visitor, 7, "trp_niemcewicz"),
	      ...
	      (jump_to_scene,"scn_krakow_kamienica_interior"),
		  (change_screen_mission),
		  (else_try),
      ...
      ("town_dungeon",
       [(check_quest_active, "qst_DostarczBron2")],
       "Poszukaj konspiratora.",
       [	   
			(1,"script_setup_troop_meeting", "trp_qdostarczbron2_2",-1),
       ],"Door to the dungeon."),
	...
	(gt, "$love_interest_in_town", 0),
      ...
      [
        ...
        (try_begin),
          ...
          (val_add, reg1, 1),
          (str_store_string, s1, "@ ({reg1} talarów za noc)"),
          (store_troop_gold, ":gold", "trp_player"),
          (lt, ":gold", reg1),
          (assign, ":can_rest", 0),
        (try_end),
      ...
      "Zaczekaj tu przez jakiś czas. {s1}",
      [
        (assign, "$auto_enter_town", "$current_town"),
        (assign, "$g_town_visit_after_rest", 1),
 ...
 but they offer you all they can find to show their gratitude.",
  ...
  (
    ...
    [
      ...
      ("trade_1",[],
       "Handluj z rusznikarzem.",
       [
           (store_sub, ":offset", "$current_town", "p_town_1"),
		   (store_add, ":merchant_troop", "trp_town_1_rusznikarz", ":offset"),
           (change_screen_trade, ":merchant_troop"),
        ]),
      ("trade_2",[],
       "Handluj z płatnerzem.",
       [
           (store_sub, ":offset", "$current_town", "p_town_1"),
		   (store_add, ":merchant_troop", "trp_town_1_platnerz", ":offset"),
           (change_screen_trade, ":merchant_troop"),
        ]),
      ("trade_3",[],
       "Handluj z handlarzem strojami.",
       [
           (store_sub, ":offset", "$current_town", "p_town_1"),
		   (store_add, ":merchant_troop", "trp_town_1_stroje", ":offset"),
           (change_screen_trade, ":merchant_troop"),
        ]),
      ("trade_4",[],
       "Handluj z handlarzm końmi.",
       [
           (store_sub, ":offset", "$current_town", "p_town_1"),
		   (store_add, ":merchant_troop", "trp_town_1_konie", ":offset"),
           (change_screen_trade, ":merchant_troop"),
        ]),
...
# (call_script, "script_get_max_skill_of_player_party", "skl_trade"),
  ...
  (
    ...
    "W przebraniu udaje ci się przedostać do miasta.",
    "none",
    [],
    [
      ("continue",[],"Kontynuuj...",
...
#####################################################################
 ...
 You get up and try to look for any other survivors from your party.",
  ...
  (
    ...
    "Zostajesz pojmany jako więzień",
    "none",
    [
       (assign, "$g_player_is_captive", 1),
       (assign,"$auto_menu",-1),
  ...
  (
    ...
    "Twoi wrogowie biorą cię do niewoli.",
    "none",
    [
  ...
  (
    ...
    "Po wielu bolesnych dniach spędzonych w niewoli, trafiasz na okazję i uciekasz swoim prześladowcom!",
    "none",
    [
  ...
  (
    ...
    "Otaczają cię straże i odbierają broń. Klnąc i ubliżając ci, wtrącają cię do więzienia, gdzie musisz spędzić żałosne dni swojej niewoli.",
    "none",
    [
        (troop_get_type, ":is_female", "trp_player"),
        (try_begin),
 ...
 The guards are outraged and beat you savagely before throwing you back into the cell for God knows how long...",
  ...
  (
    ...
    "Po wielu dniach aresztu, wreszcie zostałeś uwolniony.",#After days of imprisonment, you are finally set free when your captors exchange you with another prisoner.
    "none",
    [
      (play_cue_track, "track_escape"),
      ],
  ...
  (
    ...
    "Jeden ze strażników złożył ci ofertę przywrócenia wolności w zamian za {reg5} złociszy. Decydujesz...",
    "none",
    [
...
#		(str_store_faction_name, s9, "$players_kingdom"),
  ...
  (
    ...
    "Zdrada!^^{s1} opuścił: {s2} i dołączył do: {s3}.",
    "none",
    [
  ...
  (
    ...
    "Dobre wieści!^^{s1} opuścił: {s2} i dołączył do: {s3}.",
    "none",
    [
      (str_store_troop_name, s1, "$g_notification_menu_var1"),
      (str_store_faction_name, s2, "$g_notification_menu_var2"),
  ...
  (
    ...
    "Deklaracja wojny!^^{s1} i {s2} wchodzą w stan wojny!",
    "none",
    [
...
#      (else_try),
  ...
  (
    ...
    "Traktat pokojowy!^^{s1} i {s2} zawierają pokój!^{s57}",
    "none",
    [
  ...
  (
    ...
    "Frakcja wyeliminowana!^^{s1} przestaje istnieć!",
    "none",
    [
      (str_store_faction_name, s1, "$g_notification_menu_var1"),
      (set_fixed_point_multiplier, 100),
 ...
 This will surely be your best opportunity to attack him.",
  ...
  ("rozpoznanieogniem1",mnf_disable_all_keys,
    "Szpicle nie próżnowali nawet w nocy, zostaliście zatrzymani przez straże tuż po wyjściu na zewnątrz...",
    "none",[],	
    [ 
      ("continue",[],"Kontynuuj.",
       [         
		...
		(set_visitor, 7, "trp_qrozpoznanieogniemt1"),
		(set_visitor, 2, "trp_qrozpoznanieogniemt2"),
		(set_visitor, 3, "trp_qrozpoznanieogniemt3"),
		(set_visitor, 4, "trp_qrozpoznanieogniemt3"),
		(set_visitor, 5, "trp_qrozpoznanieogniemt3"),

		(set_jump_mission,"mt_suburb_fight"),
		(jump_to_scene,":scene"),
  ...
  ("rozpoznanieogniem_fail_1",mnf_disable_all_keys,
    "Ranny w trakcie walki zostajesz pojmany i osadzony w lochu...",
    "none",[],	
    [ 
      ("continue",[],"Kontynuuj.",
       [         
	     (1,"script_2Etap_quest_failed", "qst_RozpoznanieOgniem"),
         (change_screen_map),       
  ...
  ("rozpoznanieogniem_fail_2",mnf_disable_all_keys,
    "Emisariusz zginął w walce.",
    "none",[],	
    [ 
      ("continue",[],"Kontynuuj.",
       [         
  ...
  ("rozpoznanieogniem_ok",mnf_disable_all_keys,
    "Udaje wam się wyrwać z zasadzki i opuścić przedmieścia...",
    "none",[],	
    [ 
      ("continue",[],"Kontynuuj.",
       [         
          (change_screen_map),  
		 (start_map_conversation, "trp_qrozpoznanieogniemt1"),
		 (1,"script_2Etap_quest_succeed", "qst_RozpoznanieOgniem"),
		 # (1,"script_end_quest", "qst_RozpoznanieOgniem"),
       ]),       
    ]
  ),
  
  ("rozpoznanieogniem_drzwi",mnf_disable_all_keys,
    "....",
    "none",[],	
    [ 
      ("przejdz", 
      [
	  # (neq, "$KALKI", 1),
	  (quest_slot_eq, "qst_RozpoznanieOgniem", slot_quest_current_state, 1) ],
       "Wejdź do środka",
       [
		 (assign, ":scene", "scn_qemisariusz2"),
		 
		(modify_visitors_at_site,":scene"),
		(reset_visitors, 0),
		(set_visitor, 0, "trp_player"),
		(set_visitor, 1, "trp_qrozpoznanieogniemt1"),

		(set_jump_mission,"mt_suburb_fight"),
		(jump_to_scene,":scene"),
		(assign, "$talk_to", "trp_qrozpoznanieogniemt1"),
  ...
  ("powstanie",mnf_disable_all_keys,
    "Powstanie.....................",
    "none",[],	
    [ 
  ...
  ("koniecgry",mnf_disable_all_keys,
    "Odniosłeś zwycięstwo!.",
    "none",[
	(try_begin),
	(neq,"$PresentationGameEND",1),
	(start_presentation, "prsnt_koniec_gry"),
	(try_end),
	],	
    [ 

      ("continue",[],"Zakoncz gre.",
       [         
          (change_screen_map),     
       ]),       
    ]
  ...
  ("break_menu",mnf_disable_all_keys,
    "empty",
    "none",[
	(change_screen_map),
	],	
  ...
  ("quest_end",mnf_disable_all_keys,
    "empty",
    "none",[
	(change_screen_return),
	(try_begin),
	(neq, "$break_menu",1),
	(start_presentation, "prsnt_quest_end"),
	(try_end),
	(assign, "$break_menu",0),
	],	
    [ 
  ...
  ("GMRegiments",mnf_disable_all_keys,
    "empty",
    "none",[
	(change_screen_return),
	(try_begin),
	   (neq, "$break_menu",1),
         (set_shader_param_int, "@disableUI", 1),
         (assign, "$duel_type", 1),
         (assign, "$currentRecruitTroop", -1),
		 (store_sub, ":offset", "$g_encountered_party", "p_wojsko_1"),
         (assign, ":scene", "scn_etap_III_oboz_a"),
		 (val_add, ":scene", ":offset"),
         (modify_visitors_at_site,":scene"),
           (reset_visitors),
           (set_visitor, 0, "trp_player"),
           (set_jump_mission,"mt_recruitment"),
           (jump_to_scene,":scene"),
         (change_screen_mission),
  	   	 (assign, "$break_menu", 1),
	(else_try),
    ...
    ("GMRegiments2",mnf_disable_all_keys,
    "empty",
    "none",[
	(change_screen_return),
	(try_begin),
	...
	(else_try),
		(eq, "$Rosja",1),
		(jump_to_menu, "mnu_rosja_camp"),
	(else_try),
		(eq, "$Wegry",1),
		(jump_to_menu, "mnu_wegry_camp"),
	(try_end),
	# (start_presentation, "prsnt_regiments"),
	],	
    [ 
...
(change_screen_quit),    
    ...
    ("koniec_rewolucji",mnf_disable_all_keys,
    "{s10}",
    "none",[  
	],	
    [ 
      ...
      ("continue",[],"Kontynuuj.",
       [         
	   	# (party_set_flags, "p_wegry_camp",0x00000100,1),
	(party_set_flags, "p_rosja_camp",0x00000100,1),
           (change_screen_return),
       ]),       
    ]
  ),
  ...
  (
    "rosja_camp",mnf_disable_all_keys,
    "Obóz rewolucjonistów węgierskich.",
    "none",
    [
	# (try_begin),
		# (check_quest_active, "qst_Oboz_weg"),
		# (1,"script_fail_quest","qst_Oboz_weg"),
		# (1,"script_end_quest","qst_Oboz_weg"),
	# (else_try),
		# (check_quest_active, "qst_Oboz_ros"),
		# (1,"script_succeed_quest","qst_Oboz_ros"),
		# (1,"script_end_quest","qst_Oboz_ros"),
	# (try_end),
	],
    [
      ("enter",[(eq,0,1)],"Wejdź.",[
      # (modify_visitors_at_site,"scn_russian_camp"),
      (reset_visitors, 0),
	  (set_visitor, 0, "trp_zaopatrzeniowiec_1"),
	  (set_visitor, 1, "$ObozDowodca"),
	  (set_visitor, 2, "trp_player"),
	  
	  ###8 straznikow
	  (set_visitor, 3, "trp_camp_troops"),
	  (set_visitor, 4, "trp_polish_fusilier"),
	  (set_visitor, 5, "trp_camp_troops"),
	  (set_visitor, 6, "trp_camp_troops"),
	  (set_visitor, 7, "trp_polish_fusilier"),
	  (set_visitor, 8, "trp_polish_fusilier"),
	  (set_visitor, 9, "trp_camp_troops"),
	  (set_visitor, 10, "trp_polish_fusilier"),
	  
	  ###10 straznikow
	  (set_visitor, 11, "trp_camp_troops"),
	  (set_visitor, 12, "trp_camp_troops"),
	  (set_visitor, 13, "trp_camp_troops"),
	  (set_visitor, 14, "trp_camp_troops"),
	  (set_visitor, 15, "trp_camp_troops"),
	  (set_visitor, 16, "trp_camp_troops"),
	  (set_visitor, 17, "trp_camp_troops"),
	  (set_visitor, 18, "trp_camp_troops"),
	  (set_visitor, 19, "trp_camp_troops"),
	  (set_visitor, 20, "trp_camp_troops"),
	  
	  (set_visitor, 21, "trp_tukaj"),
	  (assign,"$Oboz",1),
	  (set_jump_mission,"mt_camp"),
	  # (jump_to_scene,"scn_russian_camp"),
	  (change_screen_mission)]),
      ("talkto",[],"Przeprowadź rozmowę...",[
	  (jump_to_menu, "mnu_camp_talk_to")]),
	  
      ("czekaj",[],"Zostań tu przez jakiś czas.",[
	  
	  (rest_for_hours_interactive, 200, 4, 0),
	  (change_screen_return),
	  ]),
	  
      ("leave",[],"Opuść.",[(assign,"$Oboz",0),(leave_encounter),(change_screen_return)]),
    ]
  ),
  (
    "wegry_camp",0,
	...
	# (else_try),
		# (check_quest_active, "qst_Oboz_ros"),
		# (1,"script_fail_quest","qst_Oboz_ros"),
		# (1,"script_end_quest","qst_Oboz_ros"),
	# (try_end),
	],
    [
      ("enter",[(eq,0,1)],"Wejdź.",[
      (modify_visitors_at_site,"scn_hungarian_camp"),
      (reset_visitors, 0),
	  (set_visitor, 0, "trp_zaopatrzeniowiec_2"),
	  (set_visitor, 1, "trp_dowodca3"),
	  (set_visitor, 2, "trp_player"),
	  ...
	  (assign,"$Oboz",1),
	  (set_jump_mission,"mt_camp"),
	  (jump_to_scene,"scn_hungarian_camp"),
	  (change_screen_mission)]),
    ...
    [
      ...
      ("leader",[(eq, "$GeneralAvailable",1)],"Przywódca.",[
	  # (assign, "$Wegry", 1),
	  # (try_begin),
		 # (eq, "$Wegry",1),
		 (modify_visitors_at_site,"scn_hungarian_camp"),
	  ...
	  (change_screen_map_conversation, "trp_dowodca3"),
	  ]),
	  
	  
      ("werbownik",[(eq,0,1)],"Werbownik.",[
	  (neq,"$Tutorial_Werbownik", 0),
	  (set_jump_mission, "mt_conversation_encounter"),
	  (try_begin),
		 (eq, "$Wegry",1),
		 (modify_visitors_at_site,"scn_hungarian_camp"),
	 ...
	 (set_jump_mission, "mt_conversation_encounter"),
	  ...
	  (try_begin),
		(eq, "$Wegry",1),
		(set_visitor, 0, "trp_zaopatrzeniowiec_2"),
		(assign, "$talkto","trp_zaopatrzeniowiec_2"),
	  (else_try),
	    (eq, "$Rosja",1),
		(set_visitor, 0, "trp_zaopatrzeniowiec_1"),
		(assign, "$talkto","trp_zaopatrzeniowiec_1"),
	  (try_end),
	  ...
	  (change_screen_map_conversation, "$talkto"),
	  ]),
    ...
    [
      ...
      (else_try),        
        ...
        (try_begin),
          (party_slot_eq, "$g_encountered_party", slot_party_ai_substate, 0), #used in place of global variable
          (str_store_string, s3, "@Zbliżasz się do kryjówki maruderów."),
        (else_try),
          (party_slot_eq, "$g_encountered_party", slot_party_ai_substate, 1), #used in place of global variable
		  (eq, "$g_battle_won", 1),
          (str_store_string, s3, "@Wygrana!"),
		(else_try),
          (party_slot_eq, "$g_encountered_party", slot_party_ai_substate, 1), #used in place of global variable
		  (str_store_string, s3, "@Przegrana!"),
        (try_end),	
      (try_end),      
    ],
    [
	  ...
	  [
	    ...
	    # (party_get_template_id, ":template", "$g_encountered_party"),
	    (assign, "$g_enemy_party", "$g_encountered_party"),
    	(assign, ":scene_to_use","scn_marauder_camp"),
	    ...
	    # (try_for_range, ":unused", 0, ":number_of_bandits_will_be_spawned_at_each_period"),
	      # (store_random_in_range, ":random_entry_point", 2, 11),	      
	      # (set_visitor, ":random_entry_point", "trp_camp_troops", 1),	      
	    # (try_end),
      ...
      [      
        ...
        (try_begin),
          ...
          # (call_script, "script_succeed_quest", "qst_destroy_bandit_lair"),
		# (else_try),
		  (eq, ":template", "pt_kryjowka_maruderow"),
		  (check_quest_active, "qst_ZlotoMaruderow"),
		  (try_begin),
			(neq, "$Powstanie", 1),
			(call_script, "script_2Etap_quest_succeed", "qst_ZlotoMaruderow"),
		  (else_try),
		    (call_script, "script_succeed_quest", "qst_ZlotoMaruderow"),
		    (call_script, "script_end_quest", "qst_ZlotoMaruderow"),
		  (try_end),
		  # (display_message, "@Zgłoś się do naczelnika...",0x007f00),
		  (start_map_conversation, "trp_camp_troops"),
        (try_end),
      ...
      [
        ...
        (try_end),
		  ...
		  (try_begin),
			(neq, "$Powstanie", 1),
			(call_script, "script_2Etap_quest_failed", "qst_ZlotoMaruderow"),
		  (else_try),
		    (call_script, "script_fail_quest", "qst_ZlotoMaruderow"),
		    (call_script, "script_end_quest", "qst_ZlotoMaruderow"),
		  (try_end),
		
        (try_begin),
		  (party_is_active, "$g_encountered_party", 0),
          (party_get_template_id, ":template", "$g_encountered_party"),
          (neq, ":template", "pt_looter_lair"),
          (check_quest_active, "qst_destroy_bandit_lair"),
          (quest_slot_eq, "qst_destroy_bandit_lair", slot_quest_target_party, "$g_encountered_party"),
    ...
    ("zdarzenie_1",mnf_disable_all_keys,
    "4 stycznia",
    "none",[  
	],	
    [ 
    ...
    ("historical_notes",mnf_disable_all_keys,
    " ",
    "none",[  
	],	
    [ 
      ("1",[(eq,2,1)],"24 marca: Początek Insurekcji Kościuszkowskiej.",
       [         
		(assign, reg0, "str_hn_1_data"),
		(assign, reg1, "str_hn_1_naglowek"),
		(assign, reg2, "str_hn_1_tekst1"),
		(assign, reg3, "str_hn_1_tekst2"),
		(assign, reg4, "mesh_bg4"),
		(start_presentation, "prsnt_historical_note"),
       ]),       
      ("2",[(eq, 2,1)],"4 kwietnia: Bitwa pod Racławicami. ",
       [         
		(assign, reg0, "str_hn_2_data"),
		(assign, reg1, "str_hn_2_naglowek"),
		(assign, reg2, "str_hn_2_tekst1"),
		(assign, reg3, "str_hn_2_tekst2"),
		(assign, reg4, "mesh_bg5"),
		(start_presentation, "prsnt_historical_note"),
       ]),  
      ("3",[(eq, 2,1)],"17-18 kwietnia: Insurekcja Warszawska. ",
       [         
		(assign, reg0, "str_hn_3_data"),
		(assign, reg1, "str_hn_3_naglowek"),
		(assign, reg2, "str_hn_3_tekst1"),
		(assign, reg3, "str_hn_3_tekst2"),
		(assign, reg4, "mesh_bg6"),
		(start_presentation, "prsnt_historical_note"),
       ]),
      ("ret",[],"Wróć...",
       [         
  ...
  ("q_zasadzka",0,
	...
	(try_begin),
		(party_slot_eq, "$g_encountered_party", slot_party_ai_substate, 1),
		(str_store_string, s10, "@Bitwa przegrana! Wojska chroniące konwój skutecznie odparły atak."),
		...
		(try_end),
	(else_try),
		(party_slot_eq, "$g_encountered_party", slot_party_ai_substate, 2),#victory
		(str_store_string, s10, "@Bitwa wygrana! Konwój został przejęty!"),
		(quest_get_slot, ":p","qst_Zasadzka", QuestSlot+3),
		(try_begin),
			(gt, ":p", 0),
			(party_is_active, ":p"),
		...
		(try_end),
	(else_try),
		(str_store_string, s10, "@Wrogi konwój wpadł w zasadzkę, okazał się być jednak znacznie lepiej chroniony niż zwiadowcy donosili..."),
	(try_end),
	
	  (assign, ":ok",0),
	  (try_begin),
	    (quest_slot_eq, "qst_Zasadzka",slot_quest_current_state,0),
		(start_map_conversation, "trp_zasadzka"),
		(change_screen_return),
	  (else_try),
	    (assign, ":ok",1),
	  (try_end),
    ...
    [
	  ...
	  [
	    (party_set_slot, "$g_encountered_party", slot_party_ai_substate, 1),
	    (assign, "$g_enemy_party", "$g_encountered_party"),
    	(assign, ":scene_to_use","scn_zasadzka"),
        ...
        (set_battle_advantage, 0),
        (assign, "$g_battle_result", 0),
		(assign, "$SpecialBattle",1),
		(modify_visitors_at_site, ":scene_to_use"),
		(set_visitors, 4, "trp_camp_troops", 10),
        (set_jump_mission,"mt_lead_charge_maruderzy"),
        (jump_to_scene, ":scene_to_use"),
         # (call_script, "script_setup_random_scene"),     
        (change_screen_mission),        
	  ]),        
      ...
      [      
        ...
        (try_begin),
		  ...
		  (call_script, "script_2Etap_quest_succeed", "qst_Zasadzka"),
		  # (call_script, "script_end_quest", "qst_Zasadzka"),
        (try_end),
      ...
      [ 
        (try_begin),
          (check_quest_active, "qst_Zasadzka"),
          (call_script, "script_2Etap_quest_failed", "qst_Zasadzka"),
          # (call_script, "script_end_quest", "qst_Zasadzka"),
        (try_end),
  ...
  (
    "action_1",mnf_scale_picture,
    "...",
    "none",
    [
	(set_shader_param_int, "@disableUI", 0),
	(quest_set_slot, "qst_ZlotoMaruderow", QuestSlot+9, 5),
	
	(troop_set_slot, "trp_temp_array_a", 1000, 5),
	(troop_set_slot, "trp_temp_array_a", 0, "qst_ZlotoMaruderow"),
	(troop_set_slot, "trp_temp_array_a", 1, "qst_Dezercja"),
	(troop_set_slot, "trp_temp_array_a", 2, "qst_Szpieg"),
	(troop_set_slot, "trp_temp_array_a", 3, "qst_3Zdrajcy"),
	(troop_set_slot, "trp_temp_array_a", 4, "qst_Emisariusz"),
	(try_begin),
		 (neq, reg30,4412),
	 	 (eq, "$BreakPresentation", 0),
	     (assign, "$SceneViewPresentation", "prsnt_4quests"),
	     (assign, ":scene", "scn_quick_battle_1"),
	     (modify_visitors_at_site,":scene"),
         (reset_visitors),
  ...
  (
    "action_2",mnf_scale_picture,
    "...",
    "none",
    [
	(set_shader_param_int, "@disableUI", 0),
	
	(troop_set_slot, "trp_temp_array_a", 1000, 3),
	(troop_set_slot, "trp_temp_array_a", 0, "qst_Prusy"),
	(troop_set_slot, "trp_temp_array_a", 1, "qst_Zamek1"),
	(troop_set_slot, "trp_temp_array_a", 2, "qst_Kontruderzenie"),
  ...
  (
	...
	(try_begin),
		 ...
		 (neq, reg30,4412),
	 	 (eq, "$BreakPresentation", 0),
	     (assign, "$SceneViewPresentation", "prsnt_karczma"),
		   (store_sub, ":offset", "$g_encountered_party", "p_karczma_a"),
		   (assign, ":scene", "scn_karczma_a"),
		   (val_add, ":scene", ":offset"),
	     (modify_visitors_at_site,":scene"),
         (reset_visitors),
         (set_visitor, 0, "trp_player"),
		 ...
		 (set_shader_param_int, "@disableUI", 1),
		 (change_screen_mission),
		# (start_presentation, "prsnt_karczma"),
	(else_try),
		(change_screen_return),
	(try_end),
	# (eq,1,0),
        # (set_background_mesh, "mesh_bg3"),
      ],
    [
      # ("continue",[],"Enter...",
       # [
			# (store_sub, ":offset", "$g_encountered_party", "p_karczma_a"),
			# (store_add, ":scene", "scn_karczma_a", ":offset"),
			# (set_jump_mission,"mt_ai_training"),
			# (jump_to_scene, ":scene"),  
			# (change_screen_mission),        
        # ]),
  ...
  (
    ...
    [
      ("continue",[],"Kontynuuj...",
       [
		   (rest_for_hours,49,6,0),
		   (quest_set_slot, "qst_gList", slot_quest_current_state,5),
		   (assign, "$Prolog",13),
           (change_screen_map),
        ]),
	]
  ...
  ("qemisariusz_win_1",0,
    "Wrogi emisariusz poddał się, odstaw go do Warszawy gdzie zaczeka na sąd.",
    "none",
    [],
    [
      ("continue",[],"Kontynuuj...",
       [
			(party_add_prisoners, "p_main_party", "trp_qemisariuszt1",1),
			(change_screen_map),
        ]),
	]),
	
  ("qemisariusz_win_2",0,
    "Zwyciestwo z zabiciem",
    "none",
    [],
    [
      ("continue",[],"Kontynuuj...",
  ...
  ("qemisariusz_lose",0,
    ...
    [
      ("continue",[],"Kontynuuj...",
       [
			(1,"script_fail_quest", "qst_Emisariusz"),
			(1,"script_end_quest", "qst_Emisariusz"),
			(change_screen_map),
        ]),
	]),
  ...
  (
    "qinfiltracja_attack",0,
    "Bandyci nie podejrzewający zdrady pozostali w kryjówce. Rozpoczniesz atak niepostrzeżenie...",
    "none",
    [],
    [
      ("continue",[],"Kontynuuj...",
       [
			(party_add_members, "p_qinfiltracja", "trp_looter", 30),
			(assign, ":scene", "scn_quest1_hideout"),
			(modify_visitors_at_site, ":scene"),
			(reset_visitors),
			(quest_set_slot, "qst_Infiltracja", slot_quest_current_state, 16),
			(set_visitor, 0, -1),
			(set_visitor, 2, "trp_qinfiltracjat2"),
			(jump_to_scene, ":scene"),
			(set_jump_mission,"mt_qinfiltracja"),
			(change_screen_mission),
        ]),
	]
  ),
  ...
  (
    "qinfiltracja_2",0,
    "Nim opryszkowie spostrzegą się, że za długo trwa twoja nieobecność, weź oddział i zaatakuj ich kryjówkę.",
    "none",
    [
	],
    [
      ("continue",[],"Kontynuuj...",
       [
			...
			(party_get_position, pos0, "p_qinfiltracja"),
			(party_set_position, "p_main_party", pos0),
  ...
  (
    "qinfiltracja_lose",0,
    "Przegrałeś, żołnierzom udało się jednak z niemałym trudem wycofać. Bandyci chwilę później zwinęli obóz i uciekli gdzie pieprz rośnie...",
    "none",
    [],
    [
      ("continue",[],"Kontynuuj...",
       [
			...
			# (1,"script_end_quest", "qst_Infiltracja"),
			(quest_set_slot, "qst_Infiltracja", slot_quest_current_state, -1),
			(party_set_flags, "p_qinfiltracja", 0x00000100, 1),
			...
			(party_set_position, "p_main_party", pos0),
			(quest_set_slot, "qst_Infiltracja",115, 1),
			(1,"script_3Etap_quest_failed", "qst_Infiltracja"),
            (change_screen_map),
        ]),
	]
  ),
  ...
  (
    "qinfiltracja_win",0,
    "Zdołałeś wybić bandytów co do jednego, ostał się tylko ich przywódca...",
    "none",
    [],
    [
      ("continue",[],"Kontynuuj...",
       [
			(1,"script_3Etap_quest_succeed", "qst_Infiltracja"),
			(quest_set_slot, "qst_Infiltracja", slot_quest_current_state, 100),
			(party_set_flags, "p_qinfiltracja", 0x00000100, 1),
			(party_set_flags, "p_main_party", 0x00000100, 0),
			(set_camera_follow_party, "p_main_party"),
			...
			(quest_set_slot, "qst_Infiltracja",115, 1),
            (change_screen_map),
			(start_map_conversation, "trp_qinfiltracjat2"),
        ]),
	]
  ),
  ...
  (
    "qinfiltracja_fail",0,
    "Cudem udaje ci się uciec, gdy bandyci piorą się po gębach w sporze o wielkość kamienia.",
    "none",
    [],
    [
      ("continue",[],"Kontynuuj...",
  ...
  (
    "qinfiltracja_hideout",0,
    "Wraz z bandytą zbliżasz się do kryjówki...",
    "none",
    [],
    [
      ("continue",[],"Kontynuuj...",
       [
        ...
        (reset_visitors),
		# (set_visitor, 0, "trp_player"),
		(set_visitor, 1, "trp_qinfiltracjat1"),
		(set_visitor, 2, "trp_qinfiltracjat2"),
  ...
  ("qprzewrot_lose",0,
    "Dzielnie stawałeś, lecz przeciwnicy w przewadze okazali się być zbyt silni…",
    "none",
    [
	],
    [
      ("continue",[],"Kontynuuj...",
       [
			(1,"script_fail_quest", "qst_Przewrot"),
			(1,"script_end_quest", "qst_Przewrot"),
			(1,"script_change_player_honor", -2),
		   (change_screen_map),
        ]),
	]
  ),
  ...
  ("town_night",0,
    "Ulice miasta o tej porze są opustoszałe, gdzieniegdzie kręcą się tylko pijacy i przemykają ostatni przechodnie. Lepiej jednak nie włóczyć się o tej porze...",
    "none",
    [
	],
    [
  ...
  ("Zdarzenie1",0,
    "W obozie doszło do rozruchów między gołotą a przemytnikami. Mimo, iż szybko udało się ostudzić zapalczywców, usiec zdołali kilku ludzi...",
    "none",
    [
	],
    [
      ("continue",[],"Kontynuuj...",
       [
			(party_count_companions_of_type, ":t1", "p_main_party", "trp_mercenary_przemytnik"),
			(party_count_companions_of_type, ":t2", "p_main_party", "trp_mercenary_golota2"),
			
			(val_min, ":t1", 3),
			(val_min, ":t2", 3),
			(party_remove_members, "p_main_party", "trp_mercenary_przemytnik", ":t1"),
			(party_remove_members, "p_main_party", "trp_mercenary_golota2", ":t2"),
			(call_script, "script_change_player_party_morale", "p_main_party", -10),
			(change_screen_return),
        ]),
	]
  ),
  ...
  ("pduel",0,
    ...
    [
			(set_shader_param_int, "@disableUI", 1),
			(assign, ":scene", "scn_duel_1"),
			(store_sub, ":offset", "$current_town", "p_town_1"),
			(val_add, ":scene", ":offset"),
			(modify_visitors_at_site, ":scene"),
			(reset_visitors),
			(set_visitor, 0, "$duel_troop1"),
			(set_visitor, 1, "$duel_troop2"),
			...
			(else_try),
				(eq, "$duel_type", 2),
				(set_jump_mission, "mt_duel_with_lord_pistol"),
			(try_end), 
			(jump_to_scene, ":scene"),
			(change_screen_mission),
			(eq,0,1),
  ...
  (
    "gList_prison2",0,
    "Zostajesz uwolniony. Strażnik, stary gbur, nic rzec nie chciał, ponad to, że ktoś wpłacił pieniądze...",
    "none",
    [],
    [
      ("continue",[],"Kontynuuj...",
  ...
  (
    ...
    [
	(try_begin),
	(neq, "$g_leave_encounter", 1),
	(try_begin),
		(eq, "$Rosja", 1),
		(1,"script_setup_troop_meeting", "trp_ataman",-1),
	(else_try),
		(1,"script_setup_troop_meeting", "trp_werbownik_wegry",-1),
	(try_end),
	(assign, "$g_leave_encounter", 1),
	(else_try),
	(change_screen_map),
  ...
  # ]),
	...
	(try_begin),
		(eq, "$show_presentation", 1),
		(str_store_string, s2, "@Prezentacja regimentów."),
	(else_try),
		(str_store_string, s2, "@Wróć na mapę strategiczną."),
	(try_end),
	(neq, "$g_leave_encounter", 1),
	(store_sub, ":offset", "$g_encountered_party", "p_wojsko_1"),
	(store_add, ":trp", ":offset", "trp_werbownik_a"),
	(store_add, ":scene", "scn_etap_III_oboz_a", ":offset"),
	# (1,"script_setup_troop_meeting", ":trp",-1),
(modify_visitors_at_site,":scene"),(reset_visitors),
                                                  (set_visitor,8,"trp_player"),
                                                  (set_visitor,9,":trp"),
                                                  (set_jump_mission,"mt_conversation_encounter"),
                                                  (jump_to_scene,":scene"),
                                                  (change_screen_map_conversation, ":trp"),
	(assign, "$g_leave_encounter", 1),
	],
    [
      ("continue",[],"Kontynuuj...",
    ...
    [
      ("continue",[],"Kontynuuj...",
       [
		   ...
		   (assign, "$uciekinierInteraction", 1),
		   (assign, ":scene", "scn_karczma_b"),
           (modify_visitors_at_site, ":scene"),
           (reset_visitors),
		   (1,"script_tavern_set_visitors", ":scene"),
		   (jump_to_scene, ":scene"),
		   (set_jump_mission,"mt_town_default"),
		   (change_screen_mission),
        ]),
    ...
    (
    "uciekinier_duel_end",0,
    "{s20}",
    "none",
    [
	(str_clear, s20),
	(quest_set_slot, "qst_Uciekinier", slot_quest_current_state, 5),
	(try_begin),
		(eq, "$duel_result",4),
		(str_store_string, s20,"@Twój przeciwnik triumfuje. Gdy leżysz raniony, udaje mu się niepostrzeżenie odjechać..."),
		(1,"script_2Etap_quest_failed", "qst_Uciekinier"),
	  (else_try),
		(eq, "$duel_result",1),
		(quest_set_slot, "qst_Uciekinier", slot_quest_current_state, 100),
		(party_add_prisoners, "p_main_party", "trp_uciekinier", 1),
		(str_store_string, s20,"@Wygrywasz pojedynek."),
		# (change_screen_map),
		# (assign, "$uciekinierInteraction", 1),
		# (assign, ":scene", "scn_karczma_b"),
        # (modify_visitors_at_site, ":scene"),
	  ...
	  (else_try),
		(eq, "$duel_result",2),
		(str_store_string, s20,"@gave up"),	  
	(try_end),
		(quest_get_slot, ":party", "qst_Uciekinier", slot_quest_target_party),
		(remove_party, ":party"),
	],
    [
      ("continue",[],"Kontynuuj...",
    ...
    # (
    # "action_1",0,
    # "Jakiego typu akcje chesz wykonać na tym terenie?",
    # "none",
    # [
	# ],
    # [
      # ("1",[(quest_slot_eq, "qst_Dezercja", DontGiveAgain+1, 0)],"Dezercja",
       # [
			# (1,"script_2Etap_quest_start", "qst_Dezercja"),
        # ]),
      # ("2",[(quest_slot_eq, "qst_Szpieg", DontGiveAgain+1, 0)],"Szpieg",
       # [
			# (1,"script_2Etap_quest_start", "qst_Szpieg"),
        # ]),
      # ("3",[(quest_slot_eq, "qst_ZlotoMaruderow", DontGiveAgain+1, 0)],"Złoto maruderów",
       # [
			# (1,"script_2Etap_quest_start", "qst_ZlotoMaruderow"),
			# (quest_set_slot, "qst_ZlotoMaruderow", QuestSlot+9, 1),
        # ]),
      # ("4",[(quest_slot_eq, "qst_3Zdrajcy", DontGiveAgain+1, 0)],"Poplecznicy Targowicy",
       # [
			# (1,"script_2Etap_quest_start", "qst_3Zdrajcy"),
        # ]),
      # ("5",[(quest_slot_eq, "qst_Emisariusz", DontGiveAgain+1, 0)],"Emisariusz Wroga",
       # [
			# (1,"script_2Etap_quest_start", "qst_Emisariusz"),
        # ]),
      # ("none",[],"Żadne",
       # [
			# (change_screen_map),
  ...
  # ),
	...
	# ],
    # [
      # ("1",[(quest_slot_eq, "qst_Prusy", DontGiveAgain+1, 0)],"Prusy",
       # [
			# (1,"script_2Etap_quest_start", "qst_Prusy"),
        # ]),
      # ("2",[(quest_slot_eq, "qst_Zamek1", DontGiveAgain+1, 0)],"Kamień milowy",
       # [
			# (1,"script_2Etap_quest_start", "qst_Zamek1"),
        # ]),
      # ("3",[(quest_slot_eq, "qst_Kontruderzenie", DontGiveAgain+1, 0)],"Kontruderzenie",
       # [
			# (1,"script_2Etap_quest_start", "qst_Kontruderzenie"),
        # ]),
      # ("none",[],"Żadne",
       # [
			# (change_screen_map),
  ...
  # ),
    ...
    (
    "egzekucja_menu",0,
    "Wieść nagła, niczym błyskawica, zbawiła nieszczęśnika przytomności. Aby jednak zadośćuczynić prośbie, dostarcz go do Martinovicsa.",
    "none",
    [
	],
    [
     ...
     (
    "poslaniec_debreczyn",0,
    "Spotykasz posłańca z listem od przywódcy konspiratorów węgierskich.",
    "none",
    [
	(try_begin),
		(quest_slot_eq, "qst_2Etap", slot_quest_current_state, 4),
		(change_screen_map),
	(try_end),
	],
    [
      ("continue",[],"Kontynuuj",
       [
			...
			(str_store_string, s100, "@{playername}, bez zwłoki przybywaj do Budapesztu. Mam ci do przekazania wieści wielkiej wagi.^^^														Ignac Martinovics "),
			(start_presentation, "prsnt_letter"),
			(1,"script_quest_update", "qst_2Etap", 4),
     ...
     (
    "2etap_koniec",0,
    "Udaj się do Rzeczpospolitej i odnajdź Generała Tadeusza Kościuszkę, naczelnika powstania. ",
    "none",
    [
	],
    [
      ("continue",[],"Kontynuuj",
       [
			(1,"script_end_quest", "qst_2Etap"),
			(assign, "$IgnacAvailable", 0),
			(1,"script_start_quest", "qst_3Etap",0),
			(1,"script_quest_update", "qst_3Etap", 0),
			(1,"script_powstanie"),
			(change_screen_map),
        ]),
	]
	...
	(try_begin),
		(eq, "$duel_result", 1),
		(str_store_string, s20, "@Błyskotliwą prowokację, zakończyło równie błyskotliwe zwycięstwo. Oponent padł bez życia."),
		(1,"script_2Etap_quest_succeed", "qst_Zniewaga"),
	(else_try),
		(str_store_string, s20, "@Mimo błyskotliwej prowokacji, oponent zatriumfował w starciu. "),
		(1,"script_2Etap_quest_failed", "qst_Zniewaga"),
	(try_end),
	],
    [
      ("continue",[],"Kontynuuj",
    ...
    (
    "speech_fight_success",0,
    "Udało ci się pokonać przeciwników i opuścić karczmę, musisz jednak uciekać nim okolica zaroi się od wojska...",
    "none",
    [
	],
    [
    ...
    (
    "speech_fight_fail",0,
    "Nie udało ci się pokonać przeciwników, zostałeś raniony i wzięty do niewoli...",
    "none",
    [
	],
    [
      ("continue",[],"Kontynuuj",
       [
			(try_begin),
				...
				(assign, ":fac", "fac_kingdom_4"),
			(else_try),
				(check_quest_active, "qst_Uniwersal"),
				(assign, ":quest", "qst_Uniwersal"),
				(assign, ":fac", Targowica),
			(try_end),
			(quest_set_slot, ":quest", slot_quest_current_state, 100),
			(1,"script_2Etap_quest_failed", ":quest"),

			(1,"script_get_closest_town_of_faction", "p_main_party", ":fac"),
			(try_begin),
				(eq, reg0, -1),
				(1,"script_get_closest_center_of_faction", "p_main_party", ":fac"),
			(try_end),
    ...
    (
    "martinovics_koniec",0,
    "Podejrzenia Ignaca Martinovicsa okazały się słuszne. Niespełna dwie godziny później, do drzwi załomotali żandarmi i nakazali przeszukanie kamienicy. Tobie jednak udało się niepostrzeżenie wymknąć tylnymi drzwiami...",
    "none",
    [
		(1,"script_player_join_faction", "fac_kingdom_7"),
		(set_show_messages, 0),
		(1,"script_succeed_quest","qst_Oboz_weg"),
		(1,"script_end_quest","qst_Oboz_weg"),
		(1,"script_2Etap_quest_start", "qst_2Etap"),
		(set_show_messages, 1),
		(1,"script_quest_update", "qst_2Etap",1),
	],
    [
      ("continue",[],"Kontynuuj",
       [
			(jump_to_menu, "mnu_martinovics_koniec2"),
        ]),
	]
  ),
    ...
    (
    "execution",0,
    "Kupcy jadący pobliskim traktem donieśli, że w {s20} wkrótce zostaną straceni konspiratorzy...",
    "none",
    [
	],
    [
     ...
     (
    "martinovics_koniec2",0,
    "Miasto opuściłeś bezpiecznie, musisz jednak uważać na wojska habsburskie, bowiem od teraz jesteś poszukiwany...",
    "none",
    [
	],
    [
  ...
  (
    "4etap_town_weakened",0,
    "{s0}",
    "none",
    [
		(party_get_slot, ":quest", "$Etap4Region", Town4Etap),
		
		(try_begin),
			(eq, ":quest", "qst_Egzekucja"),
			(str_store_string, s0, "@Dzięki sprawnej akcji odbicia spiskowców, ich dywersyjny atak na budynki garnizonu powiódł się. Możliwości obronne miasta zostały znacznie osłabione."),
		(else_try),
			(eq, ":quest", "qst_Prusy"),
			(str_store_string, s0, "@Na przedmieściach wybuchły zamieszki. Część wojska należąca do garnizonu, została wysłana w celu stłumienia rozruchów, osłabiając tym samym potencjał obronny miasta."),
		(else_try),
			(eq, ":quest", "qst_1000Zdrajca"),
			(str_store_string, s0, "@Atak na dowódcę garnizonu obniżył morale i dyscyplinę załogi miasta. Nastąpiła masowa dezercja."),
		(try_end),
	],
    [
      ("continue",[],"Kontynuuj",
       [
			(party_get_num_companion_stacks, ":num", "$Etap4Region"),
			(try_for_range, ":i", 0, ":num"),
				(party_stack_get_size, ":size", "$Etap4Region", ":i"),
				(party_stack_get_troop_id, ":troop", "$Etap4Region", ":i"),
				(val_div, ":size", 2),
				(party_remove_members, "$Etap4Region", ":troop", ":size"),
			(try_end),
    ...
    "none",
    [
	   (start_presentation, "prsnt_Etap4End"),
	],
    [
      ("continue",[],"Kontynuuj",
       [
    ...
    [
		(try_begin),
			(troop_slot_eq, "trp_dowodca4", TroopDialogLevel, 0),
			# (1,"script_setup_troop_meeting", "trp_dowodca4_guard",-1),
			
			(assign, ":meeting_troop", "trp_dowodca4_guard"),
			(assign, ":meeting_scene", "scn_zendar_arena"),
		  (modify_visitors_at_site,":meeting_scene"),
		  (reset_visitors),
		  (set_visitor,0,"trp_player"),
    ...
    [
      ...
      ("enter",[(eq, "$PriestAvailable",1)],"Porozmawiaj z księdzem.",
       [
			(assign, ":meeting_troop", "trp_dowodca4"),
			(assign, ":meeting_scene", "scn_zendar_arena"),
		  (modify_visitors_at_site,":meeting_scene"),
		  (reset_visitors),
		  (set_visitor,0,"trp_player"),
...
sorted_troops = sorted(troops)
for i in range(len(sorted_troops)):
  current_troop = list(sorted_troops[i])
  current_troop[1] = sorted_troops[i][1]
  sorted_troops[i] = tuple(current_troop)

choose_troop = Game_Menu_Template(
  id="choose_troops_",
  text="Rozmawiaj z: (Strona {current_page} z {num_pages})",
  optn_id="choose_troop_",
  optn_text="{list_item[1]}",
  optn_consq = [
    (call_script, "script_setup_troop_meeting", "trp_{list_item[0]}",-1),
  ]
)

game_menus += choose_troop.generate_menus(sorted_troops) 
...
choose_scene_template = Game_Menu_Template(
  id="choose_scenes_",
  text="Scena: (Strona {current_page} z {num_pages})",
  optn_id="choose_scene_",
  optn_text="{list_item[0]}",
  optn_consq = [
    (set_jump_mission, "mt_ai_training"),
